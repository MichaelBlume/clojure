...
public class PersistentArrayMap extends APersistentMap implements IObj,
    IEditableCollection {
  ...
  static public PersistentArrayMap createWithCheck(Object[] init) {
    for (int i = 0; i < init.length; i += 2) {
      for (int j = i + 2; j < init.length; j += 2) {
        if (equalKey(init[i], init[j])) throw new IllegalArgumentException(
  ...
  static public PersistentArrayMap createAsIfByAssoc(Object[] init) {
    ...
    if (n < init.length) {
      ...
      if (m != n) throw new IllegalArgumentException("Internal error: m=" + m);
      init = nodups;
    }
    return new PersistentArrayMap(init);
  }
  ...
  public IMapEntry entryAt(Object key) {
    int i = indexOf(key);
    if (i >= 0) return new MapEntry(array[i], array[i + 1]);
    return null;
  }
  ...
  public IPersistentMap assocEx(Object key, Object val) {
    ...
    {
      if (array.length > HASHTABLE_THRESHOLD) return createHT(array).assocEx(
          key, val);
      newArray = new Object[array.length + 2];
      if (array.length > 0) System.arraycopy(array, 0, newArray, 2,
          array.length);
      newArray[0] = key;
      newArray[1] = val;
    }
  ...
  public IPersistentMap assoc(Object key, Object val) {
    ...
    {
      if (array.length > HASHTABLE_THRESHOLD) return createHT(array).assoc(key,
          val);
      newArray = new Object[array.length + 2];
      if (array.length > 0) System.arraycopy(array, 0, newArray, 2,
          array.length);
  ...
  private int indexOfObject(Object key) {
    ...
    for (int i = 0; i < array.length; i += 2) {
      if (ep.equiv(key, array[i])) return i;
    }
    return -1;
  }
  ...
  private int indexOf(Object key) {
    if (key instanceof Keyword) {
      ...
      return -1;
    } else return indexOfObject(key);
  }

  static boolean equalKey(Object k1, Object k2) {
    if (k1 instanceof Keyword) return k1 == k2;
  ...
  public ISeq seq() {
    if (array.length > 0) return new Seq(array, 0);
    return null;
  }
  ...
  static class Seq extends ASeq implements Counted {
    ...
    public ISeq next() {
      if (i + 2 < array.length) return new Seq(array, i + 2);
      return null;
    }
  ...
  static final class TransientArrayMap extends ATransientMap {
    ...
    ITransientMap doAssoc(Object key, Object val) {
      ...
      {
        if (len >= array.length) return PersistentHashMap.create(array)
            .asTransient().assoc(key, val);
        array[len++] = key;
        array[len++] = val;
      }
      return this;
