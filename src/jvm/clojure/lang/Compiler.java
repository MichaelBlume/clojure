...
public class Compiler implements Opcodes {
  ...
  static Symbol resolveSymbol(Symbol sym) {
    ...
    if (sym.name.indexOf('.') > 0) return sym;
    if (sym.ns != null) {
      Namespace ns = namespaceFor(sym);
      if (ns == null || ns.name.name == sym.ns) return sym;
      return Symbol.intern(ns.name.name, sym.name);
    }
    Object o = currentNS().getMapping(sym);
    if (o == null) return Symbol.intern(currentNS().name.name, sym.name);
    else if (o instanceof Class) return Symbol.intern(null,
        ((Class) o).getName());
  ...
  static class DefExpr implements Expr {
    ...
    public Object eval() {
      try {
        ...
        if (!(e instanceof CompilerException)) throw new CompilerException(
            source, line, column, e);
        else throw (CompilerException) e;
      }
    }
    ...
    public void emit(C context, ObjExpr objx, GeneratorAdapter gen) {
      ...
      if (initProvided) {
        ...
        } else init.emit(C.EXPRESSION, objx, gen);
    ...
    static class Parser implements IParser {
      public Expr parse(C context, Object form) {
        ...
        if (RT.count(form) > 3) throw Util
            .runtimeException("Too many arguments to def");
        else if (RT.count(form) < 2) throw Util
            .runtimeException("Too few arguments to def");
        else if (!(RT.second(form) instanceof Symbol)) throw Util
            .runtimeException("First argument to def must be a Symbol");
        Symbol sym = (Symbol) RT.second(form);
        Var v = lookupVar(sym, true);
        if (v == null) throw Util
            .runtimeException("Can't refer to qualified var that doesn't exist");
        if (!v.ns.equals(currentNS())) {
          if (sym.ns == null) v = currentNS().intern(sym);
        ...
        if (docstring != null) mm = (IPersistentMap) RT.assoc(mm, RT.DOC_KEY,
            docstring);
        ...
        return new DefExpr((String) SOURCE.deref(), (Integer) LINE.deref(),
            (Integer) COLUMN.deref(), v, analyze(context == C.EVAL ? context
                : C.EXPRESSION, RT.third(form), v.sym.name), meta,
            RT.count(form) == 3, isDynamic);
      }
    }
  }
  ...
  public static class AssignExpr implements Expr {
    ...
    static class Parser implements IParser {
      public Expr parse(C context, Object frm) {
        ISeq form = (ISeq) frm;
        if (RT.length(form) != 3) throw new IllegalArgumentException(
            "Malformed assignment, expecting (set! target val)");
        Expr target = analyze(C.EXPRESSION, RT.second(form));
        if (!(target instanceof AssignableExpr)) throw new IllegalArgumentException(
  ...
  public static class TheVarExpr implements Expr {
    ...
    static class Parser implements IParser {
      public Expr parse(C context, Object form) {
        ...
        if (v != null) return new TheVarExpr(v);
  ...
  static public abstract class HostExpr implements Expr, MaybePrimitiveExpr {
    ...
    public static void emitBoxReturn(ObjExpr objx, GeneratorAdapter gen,
      ...
      if (returnType.isPrimitive()) {
        if (returnType == boolean.class) {
          ...
          } else if (returnType == double.class) gen.invokeStatic(DOUBLE_TYPE,
              doubleValueOfMethod);
          else if (returnType == long.class) gen.invokeStatic(NUMBERS_TYPE,
              Method.getMethod("Number num(long)"));
          else if (returnType == byte.class) gen.invokeStatic(BYTE_TYPE,
              byteValueOfMethod);
          else if (returnType == short.class) gen.invokeStatic(SHORT_TYPE,
              shortValueOfMethod);
        }
      }
    }
      ...
      if (paramType.isPrimitive()) {
        if (paramType == boolean.class) {
          ...
          if (RT.booleanCast(RT.UNCHECKED_MATH.deref())) {
            if (paramType == int.class) m = Method
                .getMethod("int uncheckedIntCast(Object)");
            else if (paramType == float.class) m = Method
                .getMethod("float uncheckedFloatCast(Object)");
            else if (paramType == double.class) m = Method
                .getMethod("double uncheckedDoubleCast(Object)");
            else if (paramType == long.class) m = Method
                .getMethod("long uncheckedLongCast(Object)");
            else if (paramType == byte.class) m = Method
                .getMethod("byte uncheckedByteCast(Object)");
            else if (paramType == short.class) m = Method
                .getMethod("short uncheckedShortCast(Object)");
          } else {
            if (paramType == int.class) m = Method
                .getMethod("int intCast(Object)");
            else if (paramType == float.class) m = Method
                .getMethod("float floatCast(Object)");
            else if (paramType == double.class) m = Method
                .getMethod("double doubleCast(Object)");
            else if (paramType == long.class) m = Method
                .getMethod("long longCast(Object)");
            else if (paramType == byte.class) m = Method
                .getMethod("byte byteCast(Object)");
            else if (paramType == short.class) m = Method
                .getMethod("short shortCast(Object)");
          }
    ...
    static class Parser implements IParser {
      public Expr parse(C context, Object frm) {
        ...
        Expr instance = null;
        if (c == null) instance = analyze(context == C.EVAL ? context
        ...
        if (maybeField && !(((Symbol) RT.third(form)).name.charAt(0) == '-')) {
          ...
          if (c != null) maybeField = Reflector.getMethods(c, 0,
              munge(sym.name), true).size() == 0;
          else if (instance != null && instance.hasJavaClass()
              && instance.getJavaClass() != null) maybeField = Reflector
              .getMethods(instance.getJavaClass(), 0, munge(sym.name), false)
              .size() == 0;
        }
        ...
        if (maybeField) // field
        {
          ...
          } else return new InstanceFieldExpr(line, column, instance,
              munge(sym.name), tag);
        } else {
          ISeq call = (ISeq) ((RT.third(form) instanceof ISeq) ? RT.third(form)
              : RT.next(RT.next(form)));
          if (!(RT.first(call) instanceof Symbol)) throw new IllegalArgumentException(
          ...
          if (c != null) return new StaticMethodExpr(source, line, column, tag,
              c, munge(sym.name), args);
          else return new InstanceMethodExpr(source, line, column, tag,
              instance, munge(sym.name), args);
        }
      }
    }
    ...
    private static Class maybeClass(Object form, boolean stringOk) {
      ...
      if (form instanceof Symbol) {
        ...
        {
          if (Util.equals(sym, COMPILE_STUB_SYM.get())) return (Class) COMPILE_STUB_CLASS
              .get();
          if (sym.name.indexOf('.') > 0 || sym.name.charAt(0) == '[') c = RT
              .classForName(sym.name);
          else {
            Object o = currentNS().getMapping(sym);
      ...
      } else if (stringOk && form instanceof String) c = RT
          .classForName((String) form);
      return c;
    }
    ...
    static Class tagToClass(Object tag) {
      ...
      if (tag instanceof Symbol) {
        ...
        if (sym.ns == null) // if ns-qualified can't be classname
        {
          if (sym.name.equals("objects")) c = Object[].class;
          else if (sym.name.equals("ints")) c = int[].class;
          else if (sym.name.equals("longs")) c = long[].class;
          else if (sym.name.equals("floats")) c = float[].class;
          else if (sym.name.equals("doubles")) c = double[].class;
          else if (sym.name.equals("chars")) c = char[].class;
          else if (sym.name.equals("shorts")) c = short[].class;
          else if (sym.name.equals("bytes")) c = byte[].class;
          else if (sym.name.equals("booleans")) c = boolean[].class;
          else if (sym.name.equals("int")) c = Integer.TYPE;
          else if (sym.name.equals("long")) c = Long.TYPE;
          else if (sym.name.equals("float")) c = Float.TYPE;
          else if (sym.name.equals("double")) c = Double.TYPE;
          else if (sym.name.equals("char")) c = Character.TYPE;
          else if (sym.name.equals("short")) c = Short.TYPE;
          else if (sym.name.equals("byte")) c = Byte.TYPE;
          else if (sym.name.equals("boolean")) c = Boolean.TYPE;
        }
      }
      if (c != null) return c;
  ...
  static Class maybePrimitiveType(Expr e) {
    try {
      if (e instanceof MaybePrimitiveExpr && e.hasJavaClass()
        ...
        if (Util.isPrimitive(c)) return c;
      }
    } catch (Exception ex) {
  ...
  static Class maybeJavaClass(Collection<Expr> exprs) {
    ...
    try {
      for (Expr e : exprs) {
        ...
        if (!e.hasJavaClass()) return null;
        Class c = e.getJavaClass();
        if (match == null) match = c;
        else if (match != c) return null;
      }
    } catch (Exception e) {
      return null;
    }
      ...
      for (int i = 0; i < parameterTypes.length; i++) {
        ...
        try {
          ...
          if (primc == parameterTypes[i]) {
            ...
            final MaybePrimitiveExpr pe = (MaybePrimitiveExpr) e;
            pe.emitUnboxed(C.EXPRESSION, objx, gen);
            if (RT.booleanCast(RT.UNCHECKED_MATH.deref())) gen.invokeStatic(
                RT_TYPE, Method.getMethod("int uncheckedIntCast(long)"));
            else gen.invokeStatic(RT_TYPE,
  ...
  static class InstanceMethodExpr extends MethodExpr {
    ...
    public void emitUnboxed(C context, ObjExpr objx, GeneratorAdapter gen) {
      ...
      if (method != null) {
        ...
        if (method.getDeclaringClass().isInterface()) gen.invokeInterface(type,
            m);
        else gen.invokeVirtual(type, m);
      } else throw new UnsupportedOperationException(
          "Unboxed emit of unknown member");
    }
  ...
  static class StaticMethodExpr extends MethodExpr {
    ...
    public void emitUnboxed(C context, ObjExpr objx, GeneratorAdapter gen) {
      ...
      if (method != null) {
        ...
        if (ops != null) {
          ...
          } else gen.visitInsn((Integer) ops);
        } else {
    ...
    public void emit(C context, ObjExpr objx, GeneratorAdapter gen) {
      ...
      if (method != null) {
        ...
        if (context == C.STATEMENT) {
          if (retClass == long.class || retClass == double.class) gen.pop2();
          else if (retClass != void.class) gen.pop();
        } else {
          HostExpr.emitBoxReturn(objx, gen, method.getReturnType());
        }
      } else {
  ...
  static class NumberExpr extends LiteralExpr implements MaybePrimitiveExpr {
    ...
    public Class getJavaClass() {
      if (n instanceof Integer) return long.class;
      else if (n instanceof Double) return double.class;
      else if (n instanceof Long) return long.class;
      else throw new IllegalStateException("Unsupported Number type: "
          + n.getClass().getName());
    }
    ...
    public void emitUnboxed(C context, ObjExpr objx, GeneratorAdapter gen) {
      if (n instanceof Integer) gen.push(n.longValue());
      else if (n instanceof Double) gen.push(n.doubleValue());
      else if (n instanceof Long) gen.push(n.longValue());
    }

    static public Expr parse(Number form) {
      if (form instanceof Integer || form instanceof Double
          || form instanceof Long) return new NumberExpr(form);
      else return new ConstantExpr(form);
    }
  }
  ...
  static class ConstantExpr extends LiteralExpr {
    ...
    static class Parser implements IParser {
      public Expr parse(C context, Object form) {
        ...
        if (v == null) return NIL_EXPR;
        else if (v == Boolean.TRUE) return TRUE_EXPR;
        else if (v == Boolean.FALSE) return FALSE_EXPR;
        if (v instanceof Number) return NumberExpr.parse((Number) v);
        else if (v instanceof String) return new StringExpr((String) v);
        else if (v instanceof IPersistentCollection
            && ((IPersistentCollection) v).count() == 0) return new EmptyExpr(v);
        else return new ConstantExpr(v);
      }
    }
  }
  ...
  static class BooleanExpr extends LiteralExpr {
    ...
    public void emit(C context, ObjExpr objx, GeneratorAdapter gen) {
      if (val) gen.getStatic(BOOLEAN_OBJECT_TYPE, "TRUE", BOOLEAN_OBJECT_TYPE);
      else gen.getStatic(BOOLEAN_OBJECT_TYPE, "FALSE", BOOLEAN_OBJECT_TYPE);
      if (context == C.STATEMENT) {
        gen.pop();
      }
    }
  ...
  static class StringExpr extends LiteralExpr {
    ...
    public void emit(C context, ObjExpr objx, GeneratorAdapter gen) {
      if (context != C.STATEMENT) gen.push(str);
    }
  ...
  public static class TryExpr implements Expr {
    ...
    public void emit(C context, ObjExpr objx, GeneratorAdapter gen) {
      ...
      if (context != C.STATEMENT) gen.visitVarInsn(
          OBJECT_TYPE.getOpcode(Opcodes.ISTORE), retLocal);
      gen.mark(endTry);
      if (finallyExpr != null) finallyExpr.emit(C.STATEMENT, objx, gen);
      gen.goTo(ret);
      ...
      gen.mark(ret);
      if (context != C.STATEMENT) gen.visitVarInsn(
          OBJECT_TYPE.getOpcode(Opcodes.ILOAD), retLocal);
      gen.mark(end);
      for (int i = 0; i < catchExprs.count(); i++) {
        CatchClause clause = (CatchClause) catchExprs.nth(i);
    ...
    static class Parser implements IParser {
      ...
      public Expr parse(C context, Object frm) {
        ...
        if (context != C.RETURN) return analyze(context,
        ...
        for (ISeq fs = form.next(); fs != null; fs = fs.next()) {
          ...
          if (!Util.equals(op, CATCH) && !Util.equals(op, FINALLY)) {
            ...
            if (Util.equals(op, CATCH)) {
              ...
              if (!(RT.third(f) instanceof Symbol)) throw new IllegalArgumentException(
                  "Bad binding form, expected symbol, got: " + RT.third(f));
              Symbol sym = (Symbol) RT.third(f);
              if (sym.getNamespace() != null) throw Util
                  .runtimeException("Can't bind qualified name:" + sym);
  ...
  static class ThrowExpr extends UntypedExpr {
    ...
    static class Parser implements IParser {
      public Expr parse(C context, Object form) {
        if (context == C.EVAL) return analyze(context,
            RT.list(RT.list(FN, PersistentVector.EMPTY, form)));
    ...
    for (int i = 0; i < c1.length; i++) {
      if (c1[i] != c2[i])// || c2[i].isPrimitive() && c1[i] == Object.class))
      {
        ...
        // || Number.class.isAssignableFrom(c1[i]) && c2[i].isPrimitive()
            || c2[i].isAssignableFrom(c1[i])) better = true;
        else return false;
      }
    }
    return better;
  ...
  static int getMatchingParams(String methodName,
    ...
    for (int i = 0; i < paramlists.size(); i++) {
      ...
      for (int p = 0; match && p < argexprs.count() && aseq != null; ++p, aseq = aseq
        ...
        if (arg.hasJavaClass() && aclass == pclass) exact++;
        else match = Reflector.paramArgTypeMatch(pclass, aclass);
      }
      if (exact == argexprs.count()) {
        if (!foundExact || matchIdx == -1
            || rets.get(matchIdx).isAssignableFrom(rets.get(i))) matchIdx = i;
        tied = false;
        foundExact = true;
        ...
        if (matchIdx == -1) matchIdx = i;
          ...
          if (subsumes(paramlists.get(i), paramlists.get(matchIdx))) {
            ...
            if (rets.get(matchIdx).isAssignableFrom(rets.get(i))) matchIdx = i;
          } else if (!(subsumes(paramlists.get(matchIdx), paramlists.get(i)))) tied = true;
        }
      }
    }
    if (tied) throw new IllegalArgumentException(
  ...
  public static class NewExpr implements Expr {
    ...
    public NewExpr(Class c, IPersistentVector args, int line, int column) {
      ...
      if (ctors.isEmpty()) throw new IllegalArgumentException(
  ...
  public static class IfExpr implements Expr, MaybePrimitiveExpr {
    ...
    public Object eval() {
      ...
      if (t != null && t != Boolean.FALSE) return thenExpr.eval();
    ...
    public void doEmit(C context, ObjExpr objx, GeneratorAdapter gen,
      ...
      if (emitUnboxed) ((MaybePrimitiveExpr) thenExpr).emitUnboxed(context,
          objx, gen);
      else thenExpr.emit(context, objx, gen);
      gen.goTo(endLabel);
      gen.mark(nullLabel);
      gen.pop();
      gen.mark(falseLabel);
      if (emitUnboxed) ((MaybePrimitiveExpr) elseExpr).emitUnboxed(context,
          objx, gen);
      else elseExpr.emit(context, objx, gen);
      gen.mark(endLabel);
    }
    ...
    public Class getJavaClass() {
      ...
      if (thenClass != null) return thenClass;
    ...
    static class Parser implements IParser {
      public Expr parse(C context, Object frm) {
        ...
        if (form.count() > 4) throw Util
            .runtimeException("Too many arguments to if");
        else if (form.count() < 3) throw Util
        ...
        return new IfExpr((Integer) LINE.deref(), (Integer) COLUMN.deref(),
  ...
  public static class EmptyExpr implements Expr {
    ...
    public void emit(C context, ObjExpr objx, GeneratorAdapter gen) {
      if (coll instanceof IPersistentList) gen.getStatic(LIST_TYPE, "EMPTY",
          EMPTY_LIST_TYPE);
      else if (coll instanceof IPersistentVector) gen.getStatic(VECTOR_TYPE,
          "EMPTY", VECTOR_TYPE);
      else if (coll instanceof IPersistentMap) gen.getStatic(HASHMAP_TYPE,
          "EMPTY", HASHMAP_TYPE);
      else if (coll instanceof IPersistentSet) gen.getStatic(HASHSET_TYPE,
          "EMPTY", HASHSET_TYPE);
      else throw new UnsupportedOperationException("Unknown Collection type");
      if (context == C.STATEMENT) {
        gen.pop();
      }
    }
    ...
    public Class getJavaClass() {
      if (coll instanceof IPersistentList) return IPersistentList.class;
      else if (coll instanceof IPersistentVector) return IPersistentVector.class;
      else if (coll instanceof IPersistentMap) return IPersistentMap.class;
      else if (coll instanceof IPersistentSet) return IPersistentSet.class;
      else throw new UnsupportedOperationException("Unknown Collection type");
    }
  }
  ...
  public static class MapExpr implements Expr {
    ...
    public void emit(C context, ObjExpr objx, GeneratorAdapter gen) {
      ...
      for (int i = 0; i < keyvals.count(); i += 2) {
        ...
        if (k instanceof LiteralExpr) {
          Object kval = k.eval();
          if (constantKeys.contains(kval)) allConstantKeysUnique = false;
          else constantKeys = (IPersistentSet) constantKeys.cons(kval);
        } else allKeysConstant = false;
      }
      MethodExpr.emitArgsAsArray(keyvals, objx, gen);
      if ((allKeysConstant && allConstantKeysUnique) || (keyvals.count() <= 2)) gen
          .invokeStatic(RT_TYPE, mapUniqueKeysMethod);
      else gen.invokeStatic(RT_TYPE, mapMethod);
      if (context == C.STATEMENT) gen.pop();
    }
    ...
    static public Expr parse(C context, IPersistentMap form) {
      ...
      for (ISeq s = RT.seq(form); s != null; s = s.next()) {
        ...
        } else keysConstant = false;
        if (!(v instanceof LiteralExpr)) valsConstant = false;
      }

      Expr ret = new MapExpr(keyvals);
      if (form instanceof IObj && ((IObj) form).meta() != null) return new MetaExpr(
          ret, MapExpr.parse(context == C.EVAL ? context : C.EXPRESSION,
              ((IObj) form).meta()));
      else if (keysConstant) {
        // TBD: Add more detail to exception thrown below.
        if (!allConstantKeysUnique) throw new IllegalArgumentException(
  ...
  public static class SetExpr implements Expr {
    ...
    static public Expr parse(C context, IPersistentSet form) {
      ...
      for (ISeq s = RT.seq(form); s != null; s = s.next()) {
        ...
        if (!(expr instanceof LiteralExpr)) constant = false;
      }
  ...
  public static class VectorExpr implements Expr {
    ...
    static public Expr parse(C context, IPersistentVector form) {
      ...
      for (int i = 0; i < form.count(); i++) {
        ...
        if (!(v instanceof LiteralExpr)) constant = false;
      }
  ...
  static class StaticInvokeExpr implements Expr, MaybePrimitiveExpr {
    ...
    public void emit(C context, ObjExpr objx, GeneratorAdapter gen) {
      emitUnboxed(context, objx, gen);
      if (context != C.STATEMENT) HostExpr.emitBoxReturn(objx, gen, retClass);
      if (context == C.STATEMENT) {
        if (retClass == long.class || retClass == double.class) gen.pop2();
        else gen.pop();
      }
    ...
    public void emitUnboxed(C context, ObjExpr objx, GeneratorAdapter gen) {
      ...
      } else MethodExpr.emitTypedArgs(objx, gen, paramclasses, args);
    ...
    public static Expr parse(Var v, ISeq args, Symbol tag) {
      IPersistentCollection paramlists = (IPersistentCollection) RT.get(
          v.meta(), arglistsKey);
      if (paramlists == null) throw new IllegalStateException(
      ...
      for (ISeq aseq = RT.seq(paramlists); aseq != null; aseq = aseq.next()) {
        if (!(aseq.first() instanceof IPersistentVector)) throw new IllegalStateException(
      ...
      if (paramlist == null) throw new IllegalArgumentException(
          "Invalid arity - can't call: " + v + " with " + argcount + " args");
        ...
        if (pvar != null && PROTOCOL_CALLSITES.isBound()) {
          ...
          if (this.protocolOn != null) {
            ...
            if (methods.size() != 1) throw new IllegalArgumentException(
                "No single method: " + mname + " of interface: "
                    + protocolOn.getName() + " found for function: " + fvar.sym
    ...
    static public Expr parse(C context, ISeq form) {
      ...
      if (fexpr instanceof VarExpr && ((VarExpr) fexpr).var.equals(INSTANCE)) {
        if (RT.second(form) instanceof Symbol) {
          Class c = HostExpr.maybeClass(RT.second(form), false);
          if (c != null) return new InstanceOfExpr(c, analyze(context,
              RT.third(form)));
        }
      }
      ...
      if (fexpr instanceof KeywordExpr && RT.count(form) == 2
        ...
        return new KeywordInvokeExpr((String) SOURCE.deref(),
            (Integer) LINE.deref(), (Integer) COLUMN.deref(), tagOf(form),
      ...
      return new InvokeExpr((String) SOURCE.deref(), (Integer) LINE.deref(),
          (Integer) COLUMN.deref(), tagOf(form), fexpr, args);
    }
  }
  ...
  static public class FnExpr extends ObjExpr {
    ...
    static Expr parse(C context, ISeq form, String name) {
      ...
      if (RT.second(form) instanceof Symbol) name = ((Symbol) RT.second(form)).name;
      ...
      try {
        ...
        if (RT.second(form) instanceof IPersistentVector) form = RT.list(FN,
            RT.next(form));
        ...
        for (ISeq s = RT.next(form); s != null; s = RT.next(s)) {
          ...
          if (f.isVariadic()) {
            if (variadicMethod == null) variadicMethod = f;
            else throw Util
                .runtimeException("Can't have more than 1 variadic overload");
          } else if (methodArray[f.reqParms.count()] == null) methodArray[f.reqParms
              .count()] = f;
          else throw Util
        ...
        if (variadicMethod != null) {
          for (int i = variadicMethod.reqParms.count() + 1; i <= MAX_POSITIONAL_ARITY; i++)
            if (methodArray[i] != null) throw Util
                .runtimeException("Can't have fixed arity function with more params than variadic function");
        }

        if (fn.isStatic && fn.closes.count() > 0) throw new IllegalArgumentException(
            "static fns can't be closures");
        IPersistentCollection methods = null;
        for (int i = 0; i < methodArray.length; i++)
          if (methodArray[i] != null) methods = RT
              .conj(methods, methodArray[i]);
        if (variadicMethod != null) methods = RT.conj(methods, variadicMethod);
      ...
      if (fmeta != null) fmeta = fmeta.without(RT.LINE_KEY)
          .without(RT.COLUMN_KEY).without(RT.FILE_KEY);
  ...
  static public class ObjExpr implements Expr {
    ...
    Type[] ctorTypes() {
      ...
      for (ISeq s = RT.keys(closes); s != null; s = s.next()) {
        ...
        if (lb.getPrimitiveType() != null) tv = tv.cons(Type.getType(lb
            .getPrimitiveType()));
        else tv = tv.cons(OBJECT_TYPE);
      }
    ...
    void compile(String superName, String[] interfaceNames, boolean oneTimeUse)
      ...
      if (keywordCallsites.count() > 0) emitKeywordCallsites(clinitgen);
      ...
      for (ISeq s = RT.keys(closes); s != null; s = s.next()) {
        ...
        if (isDeftype()) {
          ...
          FieldVisitor fv;
          if (lb.getPrimitiveType() != null) fv = cv.visitField(access,
          ...
          // need it
          if (lb.getPrimitiveType() != null) cv.visitField(
              0 + (isVolatile(lb) ? ACC_VOLATILE : 0), lb.name,
              Type.getType(lb.getPrimitiveType()).getDescriptor(), null, null);
          else cv.visitField(0 // + (oneTimeUse ? 0 : ACC_FINAL)
      ...
      for (ISeq s = RT.keys(closes); s != null; s = s.next(), ++a) {
        ...
        if (primc != null) {
          ...
          if (primc == Long.TYPE || primc == Double.TYPE) ++a;
        } else {
      ...
      if (RT.booleanCast(COMPILE_FILES.deref())) writeClassFile(internalName,
          bytecode);
      // else
      // getCompiledClass();
    }
    ...
    void emitValue(Object value, GeneratorAdapter gen) {
      ...
      if (value == null) gen.visitInsn(Opcodes.ACONST_NULL);
      else if (value instanceof String) {
        gen.push((String) value);
      } else if (value instanceof Boolean) {
        if (((Boolean) value).booleanValue()) gen.getStatic(
            BOOLEAN_OBJECT_TYPE, "TRUE", BOOLEAN_OBJECT_TYPE);
        else gen.getStatic(BOOLEAN_OBJECT_TYPE, "FALSE", BOOLEAN_OBJECT_TYPE);
      ...
      // }
        ...
        if (vs == null) gen.getStatic(Type.getType(PersistentHashSet.class),
        ...
        if (cs.length() == 0) throw Util
            .runtimeException("Can't embed unreadable object in code: " + value);

        if (cs.startsWith("#<")) throw Util
            .runtimeException("Can't embed unreadable object in code: " + cs);
    ...
    public void emit(C context, ObjExpr objx, GeneratorAdapter gen) {
      ...
      if (isDeftype()) {
        ...
        gen.newInstance(objtype);
        gen.dup();
        if (supportsMeta()) gen.visitInsn(Opcodes.ACONST_NULL);
        for (ISeq s = RT.seq(closesExprs); s != null; s = s.next()) {
          LocalBindingExpr lbe = (LocalBindingExpr) s.first();
          LocalBinding lb = lbe.b;
          if (lb.getPrimitiveType() != null) objx.emitUnboxedLocal(gen, lb);
          else objx.emitLocal(gen, lb, lbe.shouldClear);
        }
        gen.invokeConstructor(objtype, new Method("<init>", Type.VOID_TYPE,
            ctorTypes()));
      }
    ...
    public void emitAssignLocal(GeneratorAdapter gen, LocalBinding lb, Expr val) {
      if (!isMutable(lb)) throw new IllegalArgumentException(
          "Cannot assign to non-mutable: " + lb.name);
      Class primc = lb.getPrimitiveType();
      gen.loadThis();
      if (primc != null) {
        if (!(val instanceof MaybePrimitiveExpr && ((MaybePrimitiveExpr) val)
            .canEmitPrimitive())) throw new IllegalArgumentException(
    ...
    private void emitLocal(GeneratorAdapter gen, LocalBinding lb, boolean clear) {
      if (closes.containsKey(lb)) {
        ...
        if (lb.isArg) {
          ...
          if (primc != null) HostExpr.emitBoxReturn(this, gen, primc);
          else {
    ...
    private void emitUnboxedLocal(GeneratorAdapter gen, LocalBinding lb) {
      ...
      if (closes.containsKey(lb)) {
        gen.loadThis();
        gen.getField(objtype, lb.name, Type.getType(primc));
      } else if (lb.isArg) gen.loadArg(lb.idx - argoff);
      else gen.visitVarInsn(Type.getType(primc).getOpcode(Opcodes.ILOAD),
          lb.idx);
    }
    ...
    Type constantType(int id) {
      ...
      if (c != null && Modifier.isPublic(c.getModifiers())) {
        ...
        if (LazySeq.class.isAssignableFrom(c)) return Type.getType(ISeq.class);
        else if (c == Keyword.class) return Type.getType(Keyword.class);
        // else if(c == KeywordCallSite.class)
        // return Type.getType(KeywordCallSite.class);
        else if (RestFn.class.isAssignableFrom(c)) return Type
            .getType(RestFn.class);
        else if (AFn.class.isAssignableFrom(c)) return Type.getType(AFn.class);
        else if (c == Var.class) return Type.getType(Var.class);
        else if (c == String.class) return Type.getType(String.class);
  ...
  public static class FnMethod extends ObjMethod {
    ...
    static public char classChar(Object x) {
      Class c = null;
      if (x instanceof Class) c = (Class) x;
      else if (x instanceof Symbol) c = primClass((Symbol) x);
      if (c == null || !c.isPrimitive()) return 'O';
    ...
    static public String primInterface(IPersistentVector arglist) {
      ...
      if (prim && arglist.count() > 4) throw new IllegalArgumentException(
          "fns taking primitives support only 4 or fewer args");
      if (prim) return "clojure.lang.IFn$" + ret;
      return null;
    }
    ...
    static FnMethod parse(ObjExpr objx, ISeq form, boolean isStatic) {
      ...
      try {
        ...
        if (pnode == null) pnode = new PathNode(PATHTYPE.PATH, null);
        ...
        if (method.retClass.isPrimitive()
            && !(method.retClass == double.class || method.retClass == long.class)) throw new IllegalArgumentException(
            "Only long and double primitives are supported");

        // register 'this' as local 0
        // registerLocal(THISFN, null, null);
        if (!isStatic) {
          if (objx.thisName != null) registerLocal(
              Symbol.intern(objx.thisName), null, null, false);
          else getAndIncLocalNum();
        }
        ...
        for (int i = 0; i < parms.count(); i++) {
          if (!(parms.nth(i) instanceof Symbol)) throw new IllegalArgumentException(
              "fn params must be Symbols");
          Symbol p = (Symbol) parms.nth(i);
          if (p.getNamespace() != null) throw Util
              .runtimeException("Can't use qualified name as parameter: " + p);
          if (p.equals(_AMP_)) {
            // if(isStatic)
            // throw Util.runtimeException("Variadic fns cannot be static");
            if (state == PSTATE.REQ) state = PSTATE.REST;
            else throw Util.runtimeException("Invalid parameter list");
          }
            ...
            // + p);
            if (pc.isPrimitive() && !(pc == double.class || pc == long.class)) throw new IllegalArgumentException(
                "Only long and double primitives are supported: " + p);

            if (state == PSTATE.REST && tagOf(p) != null) throw Util
                .runtimeException("& arg cannot have type hint");
            if (state == PSTATE.REST && method.prim != null) throw Util
                .runtimeException("fns taking primitives cannot be variadic");

            if (state == PSTATE.REST) pc = ISeq.class;
        ...
        if (method.reqParms.count() > MAX_POSITIONAL_ARITY) throw Util
            .runtimeException("Can't specify more than " + MAX_POSITIONAL_ARITY
                + " params");
        ...
        if (method.prim != null) {
          ...
          for (int i = 0; i < method.argclasses.length; i++) {
            if (method.argclasses[i] == long.class
                || method.argclasses[i] == double.class) getAndIncLocalNum();
          }
        }
    ...
    public void emit(ObjExpr fn, ClassVisitor cv) {
      if (prim != null) doEmitPrim(fn, cv);
      else if (fn.isStatic) doEmitStatic(fn, cv);
      else doEmit(fn, cv);
    }
  ...
  abstract public static class ObjMethod {
    ...
    static void emitBody(ObjExpr objx, GeneratorAdapter gen, Class retClass,
      ...
      if (Util.isPrimitive(retClass) && be.canEmitPrimitive()) {
        ...
        if (bc == retClass) be.emitUnboxed(C.RETURN, objx, gen);
        ...
        if (retClass == void.class) {
          gen.pop();
        } else gen.unbox(Type.getType(retClass));
      }
    }
  ...
  public static class LocalBinding {
    ...
    public LocalBinding(int num, Symbol sym, Symbol tag, Expr init,
      ...
      if (maybePrimitiveType(init) != null && tag != null) throw new UnsupportedOperationException(
    ...
    public boolean hasJavaClass() {
      if (init != null && init.hasJavaClass()
          ...
          && !(init instanceof MaybePrimitiveExpr)) return false;
  ...
  public static class LocalBindingExpr implements Expr, MaybePrimitiveExpr,
    ...
    public LocalBindingExpr(LocalBinding b, Symbol tag) {
      if (b.getPrimitiveType() != null && tag != null) throw new UnsupportedOperationException(
      ...
      if (b.idx > 0) {
        ...
        if (sites != null) {
          for (ISeq s = sites.seq(); s != null; s = s.next()) {
            ...
            if (common != null && common.type == PATHTYPE.PATH) o.shouldClear = false;
            // else
            // dummy = null;
          }
        }
    ...
    public void emit(C context, ObjExpr objx, GeneratorAdapter gen) {
      if (context != C.STATEMENT) objx.emitLocal(gen, b, shouldClear);
    }
    ...
    public void emitAssign(C context, ObjExpr objx, GeneratorAdapter gen,
      ...
      if (context != C.STATEMENT) objx.emitLocal(gen, b, false);
    }
    ...
    public Class getJavaClass() {
      if (tag != null) return HostExpr.tagToClass(tag);
  ...
  public static class LetFnExpr implements Expr {
    ...
    static class Parser implements IParser {
      public Expr parse(C context, Object frm) {
        ...
        if (!(RT.second(form) instanceof IPersistentVector)) throw new IllegalArgumentException(
            "Bad binding form, expected vector");

        IPersistentVector bindings = (IPersistentVector) RT.second(form);
        if ((bindings.count() % 2) != 0) throw new IllegalArgumentException(
        ...
        try {
          ...
          for (int i = 0; i < bindings.count(); i += 2) {
            if (!(bindings.nth(i) instanceof Symbol)) throw new IllegalArgumentException(
                "Bad binding form, expected symbol, got: " + bindings.nth(i));
            Symbol sym = (Symbol) bindings.nth(i);
            if (sym.getNamespace() != null) throw Util
                .runtimeException("Can't let qualified name: " + sym);
    ...
    public void emit(C context, ObjExpr objx, GeneratorAdapter gen) {
      ...
      for (ISeq bis = bindingInits.seq(); bis != null; bis = bis.next()) {
        ...
        if (lname.endsWith("__auto__")) lname += RT.nextID();
        Class primc = maybePrimitiveType(bi.init);
        if (primc != null) gen.visitLocalVariable(lname,
            Type.getDescriptor(primc), null, loopLabel, end, bi.binding.idx);
        else gen.visitLocalVariable(lname, "Ljava/lang/Object;", null,
  ...
  public static class LetExpr implements Expr, MaybePrimitiveExpr {
    ...
    static class Parser implements IParser {
      public Expr parse(C context, Object frm) {
        ...
        if (context == C.EVAL || (context == C.EXPRESSION && isLoop)) return analyze(
            context, RT.list(RT.list(FN, PersistentVector.EMPTY, form)));
        ...
        while (true) {
          ...
          if (isLoop) dynamicBindings = dynamicBindings
              .assoc(LOOP_LOCALS, null);
          ...
          try {
            ...
            for (int i = 0; i < bindings.count(); i += 2) {
              ...
              if (isLoop) {
                if (recurMismatches != null
                  ...
                  init = new StaticMethodExpr("", 0, 0, null, RT.class, "box",
                      RT.vector(init));
                  if (RT.booleanCast(RT.WARN_ON_REFLECTION.deref())) RT
                      .errPrintWriter().println("Auto-boxing loop arg: " + sym);
                } else if (maybePrimitiveType(init) == int.class) init = new StaticMethodExpr(
                    "", 0, 0, null, RT.class, "longCast", RT.vector(init));
                else if (maybePrimitiveType(init) == float.class) init = new StaticMethodExpr(
                    "", 0, 0, null, RT.class, "doubleCast", RT.vector(init));
              }
              ...
              if (isLoop) loopLocals = loopLocals.cons(lb);
            }
            if (isLoop) LOOP_LOCALS.set(loopLocals);
            Expr bodyExpr;
            ...
            if (!moreMismatches) return new LetExpr(bindingInits, bodyExpr,
                isLoop);
          } finally {
            Var.popThreadBindings();
          }
    ...
    public void doEmit(C context, ObjExpr objx, GeneratorAdapter gen,
      ...
      if (isLoop) {
        try {
          ...
          if (emitUnboxed) ((MaybePrimitiveExpr) body).emitUnboxed(context,
              objx, gen);
          else body.emit(context, objx, gen);
        } finally {
          Var.popThreadBindings();
        }
      } else {
        if (emitUnboxed) ((MaybePrimitiveExpr) body).emitUnboxed(context, objx,
            gen);
        else body.emit(context, objx, gen);
      }
      Label end = gen.mark();
      ...
      for (ISeq bis = bindingInits.seq(); bis != null; bis = bis.next()) {
        ...
        Class primc = maybePrimitiveType(bi.init);
        if (primc != null) gen.visitLocalVariable(lname,
            Type.getDescriptor(primc), null, bindingLabels.get(bi), end,
            bi.binding.idx);
        else gen.visitLocalVariable(lname, "Ljava/lang/Object;", null,
  ...
  public static class RecurExpr implements Expr {
    ...
    public void emit(C context, ObjExpr objx, GeneratorAdapter gen) {
      ...
      if (loopLabel == null) throw new IllegalStateException();
      for (int i = 0; i < loopLocals.count(); i++) {
        LocalBinding lb = (LocalBinding) loopLocals.nth(i);
        ...
        if (lb.getPrimitiveType() != null) {
          ...
          try {
            ...
            if (pc == primc) ((MaybePrimitiveExpr) arg).emitUnboxed(
                C.EXPRESSION, objx, gen);
      ...
      for (int i = loopLocals.count() - 1; i >= 0; i--) {
        LocalBinding lb = (LocalBinding) loopLocals.nth(i);
        Class primc = lb.getPrimitiveType();
        if (lb.isArg) gen.storeArg(lb.idx - (objx.isStatic ? 0 : 1));
        else {
          if (primc != null) gen.visitVarInsn(
              Type.getType(primc).getOpcode(Opcodes.ISTORE), lb.idx);
          else gen.visitVarInsn(OBJECT_TYPE.getOpcode(Opcodes.ISTORE), lb.idx);
        }
      }
    ...
    static class Parser implements IParser {
      public Expr parse(C context, Object frm) {
        ...
        if (context != C.RETURN || loopLocals == null) throw new UnsupportedOperationException(
            "Can only recur from tail position");
        if (NO_RECUR.deref() != null) throw new UnsupportedOperationException(
            "Cannot recur across try");
        PersistentVector args = PersistentVector.EMPTY;
        for (ISeq s = RT.seq(form.next()); s != null; s = s.next()) {
          args = args.cons(analyze(C.EXPRESSION, s.first()));
        }
        if (args.count() != loopLocals.count()) throw new IllegalArgumentException(
            String
                .format(
          ...
          if (primc != null) {
            ...
            if (primc == long.class) {
              if (!(pc == long.class || pc == int.class || pc == short.class
                  || pc == char.class || pc == byte.class)) mismatch = true;
            } else if (primc == double.class) {
              if (!(pc == double.class || pc == float.class)) mismatch = true;
            }
            if (mismatch) {
  ...
  private static int getAndIncLocalNum() {
    ...
    if (num > m.maxLocal) m.maxLocal = num;
  ...
  private static Expr analyze(C context, Object form, String name) {
    ...
    try {
      if (form instanceof LazySeq) {
        form = RT.seq(form);
        if (form == null) form = PersistentList.EMPTY;
      }
      if (form == null) return NIL_EXPR;
      else if (form == Boolean.TRUE) return TRUE_EXPR;
      else if (form == Boolean.FALSE) return FALSE_EXPR;
      Class fclass = form.getClass();
      if (fclass == Symbol.class) return analyzeSymbol((Symbol) form);
      else if (fclass == Keyword.class) return registerKeyword((Keyword) form);
      else if (form instanceof Number) return NumberExpr.parse((Number) form);
      else if (fclass == String.class) return new StringExpr(
          ((String) form).intern());
      ...
      // return new CharExpr((Character) form);
        ...
        if (RT.meta(form) != null) ret = new MetaExpr(ret, MapExpr.parse(
            context == C.EVAL ? context : C.EXPRESSION, ((IObj) form).meta()));
        return ret;
      } else if (form instanceof ISeq) return analyzeSeq(context, (ISeq) form,
          name);
      else if (form instanceof IPersistentVector) return VectorExpr.parse(
          context, (IPersistentVector) form);
      else if (form instanceof IRecord) return new ConstantExpr(form);
      else if (form instanceof IType) return new ConstantExpr(form);
      else if (form instanceof IPersistentMap) return MapExpr.parse(context,
          (IPersistentMap) form);
      else if (form instanceof IPersistentSet) return SetExpr.parse(context,
          (IPersistentSet) form);
      ...
      if (!(e instanceof CompilerException)) throw new CompilerException(
          (String) SOURCE_PATH.deref(), (Integer) LINE.deref(),
          (Integer) COLUMN.deref(), e);
      else throw (CompilerException) e;
    }
  }
  ...
  static public Var isMacro(Object op) {
    ...
    if (op instanceof Symbol && referenceLocal((Symbol) op) != null) return null;
    if (op instanceof Symbol || op instanceof Var) {
      Var v = (op instanceof Var) ? (Var) op : lookupVar((Symbol) op, false,
          false);
      if (v != null && v.isMacro()) {
        if (v.ns != currentNS() && !v.isPublic()) throw new IllegalStateException(
            "var: " + v + " is not public");
        return v;
      }
    }
    ...
    if (op instanceof Symbol || op instanceof Var) {
      ...
      if (v != null) {
        ...
        if (ret != null) {
          ...
          if (arityPred == null || RT.booleanCast(arityPred.invoke(arity))) return ret;
        }
      }
    }
    return null;
      ...
      if (v != null) {
        ...
        if (op instanceof Symbol) {
          ...
          if (sym.name.charAt(0) == '.') {
            if (RT.length(form) < 2) throw new IllegalArgumentException(
            ...
            // else
            if (idx == sname.length() - 1) return RT.listStar(NEW,
                Symbol.intern(sname.substring(0, idx)), form.next());
          }
        }
      }
    }
  ...
  static Object macroexpand(Object form) {
    ...
    if (exf != form) return macroexpand(exf);
    return form;
  }
  ...
  private static Expr analyzeSeq(C context, ISeq form, String name) {
    ...
    if (RT.meta(form) != null && RT.meta(form).containsKey(RT.LINE_KEY)) line = (Integer) RT
        .meta(form).valAt(RT.LINE_KEY);
    if (RT.meta(form) != null && RT.meta(form).containsKey(RT.COLUMN_KEY)) column = (Integer) RT
        .meta(form).valAt(RT.COLUMN_KEY);
    Var.pushThreadBindings(RT.map(LINE, line, COLUMN, column));
    try {
      Object me = macroexpand1(form);
      if (me != form) return analyze(context, me, name);

      Object op = RT.first(form);
      if (op == null) throw new IllegalArgumentException("Can't call nil");
      IFn inline = isInline(op, RT.count(RT.next(form)));
      if (inline != null) return analyze(context,
          preserveTag(form, inline.applyTo(RT.next(form))));
      IParser p;
      if (op.equals(FN)) return FnExpr.parse(context, form, name);
      else if ((p = (IParser) specials.valAt(op)) != null) return p.parse(
          context, form);
      else return InvokeExpr.parse(context, form);
    } catch (Throwable e) {
      if (!(e instanceof CompilerException)) throw new CompilerException(
          (String) SOURCE_PATH.deref(), (Integer) LINE.deref(),
          (Integer) COLUMN.deref(), e);
  ...
  public static Object eval(Object form, boolean freshLoader) {
    ...
    try {
      ...
      try {
        ...
        if (!(e instanceof RuntimeException)) throw Util.sneakyThrow(e);
      ...
      if (createdLoader) Var.popThreadBindings();
    }
  }

  private static int registerConstant(Object o) {
    if (!CONSTANTS.isBound()) return -1;
  ...
  private static KeywordExpr registerKeyword(Keyword keyword) {
    if (!KEYWORDS.isBound()) return new KeywordExpr(keyword);
  ...
  private static int registerKeywordCallsite(Keyword keyword) {
    if (!KEYWORD_CALLSITES.isBound()) throw new IllegalAccessError(
        "KEYWORD_CALLSITES is not bound");
  ...
  private static int registerProtocolCallsite(Var v) {
    if (!PROTOCOL_CALLSITES.isBound()) throw new IllegalAccessError(
        "PROTOCOL_CALLSITES is not bound");
  ...
  private static void registerVarCallsite(Var v) {
    if (!VAR_CALLSITES.isBound()) throw new IllegalAccessError(
        "VAR_CALLSITES is not bound");
  ...
  static PathNode commonPath(PathNode n1, PathNode n2) {
    ISeq xp = fwdPath(n1);
    ISeq yp = fwdPath(n2);
    if (RT.first(xp) != RT.first(yp)) return null;
  ...
  static void addAnnotation(Object visitor, IPersistentMap meta) {
    try {
      if (meta != null && ADD_ANNOTATIONS.isBound()) ADD_ANNOTATIONS.invoke(
          visitor, meta);
    } catch (Exception e) {
      throw Util.sneakyThrow(e);
    }
  ...
  private static Expr analyzeSymbol(Symbol sym) {
    ...
    if (sym.ns == null) // ns-qualified syms are always Vars
    {
      ...
      if (namespaceFor(sym) == null) {
        ...
        if (c != null) {
          if (Reflector.getField(c, sym.name, true) != null) return new StaticFieldExpr(
              (Integer) LINE.deref(), (Integer) COLUMN.deref(), c, sym.name,
              tag);
    ...
    if (o instanceof Var) {
      Var v = (Var) o;
      if (isMacro(v) != null) throw Util
          .runtimeException("Can't take value of a macro: " + v);
      if (RT.booleanCast(RT.get(v.meta(), RT.CONST_KEY))) return analyze(
          C.EXPRESSION, RT.list(QUOTE, v.get()));
      registerVar(v);
      return new VarExpr(v, tag);
  ...
  static String destubClassName(String className) {
    // skip over prefix + '.' or '/'
    if (className.startsWith(COMPILE_STUB_PREFIX)) return className
        .substring(COMPILE_STUB_PREFIX.length() + 1);
    return className;
  }

  static Type getType(Class c) {
    String descriptor = Type.getType(c).getDescriptor();
    if (descriptor.startsWith("L")) descriptor = "L"
        + destubClassName(descriptor.substring(1));
  ...
  static public Object resolveIn(Namespace n, Symbol sym, boolean allowPrivate) {
    ...
    if (sym.ns != null) {
      ...
      if (ns == null) throw Util.runtimeException("No such namespace: "
          + sym.ns);

      Var v = ns.findInternedVar(Symbol.intern(sym.name));
      if (v == null) throw Util.runtimeException("No such var: " + sym);
      else if (v.ns != currentNS() && !v.isPublic() && !allowPrivate) throw new IllegalStateException(
      ...
      if (Util.equals(sym, COMPILE_STUB_SYM.get())) return COMPILE_STUB_CLASS
          .get();
  ...
  static Var lookupVar(Symbol sym, boolean internNew, boolean registerMacro) {
    ...
    if (sym.ns != null) {
      ...
      if (o == null) {
        ...
        if (internNew) var = currentNS().intern(Symbol.intern(sym.name));
      } else if (o instanceof Var) {
        var = (Var) o;
      } else {
    ...
    if (var != null && (!var.isMacro() || registerMacro)) registerVar(var);
    return var;
  }
  ...
  static LocalBinding referenceLocal(Symbol sym) {
    if (!LOCAL_ENV.isBound()) return null;
  ...
  public static Object load(Reader rdr, String sourcePath, String sourceName) {
    ...
    Var.pushThreadBindings(RT.mapUniqueKeys(LOADER, RT.makeClassLoader(),
        ...
        null, LOOP_LOCALS, null, NEXT_LOCAL_NUM, 0, RT.CURRENT_NS,
  ...
  static public void writeClassFile(String internalName, byte[] bytecode)
    ...
    if (genPath == null) throw Util.runtimeException("*compile-path* not set");
  ...
  public static void pushNSandLoader(ClassLoader loader) {
    Var.pushThreadBindings(RT.map(
        Var.intern(Symbol.intern("clojure.core"), Symbol.intern("*ns*"))
            .setDynamic(), null, RT.FN_LOADER_VAR, loader));
  }
  ...
  public static Object compile(Reader rdr, String sourcePath, String sourceName)
      throws IOException {
    if (COMPILE_PATH.deref() == null) throw Util
        .runtimeException("*compile-path* not set");
    ...
    Var.pushThreadBindings(RT.mapUniqueKeys(SOURCE_PATH, sourcePath, SOURCE,
        ...
        NEXT_LOCAL_NUM, 0, RT.CURRENT_NS, RT.CURRENT_NS.deref(), LINE_BEFORE,
    ...
    try {
      ...
      if (objx.constants.count() % INITS_PER != 0) ++numInits;
  ...
  static public class NewInstanceExpr extends ObjExpr {
    ...
    static class ReifyParser implements IParser {
      public Expr parse(C context, Object frm) {
        ...
        if (frm instanceof IObj && ((IObj) frm).meta() != null) return new MetaExpr(
            ret, MapExpr.parse(context == C.EVAL ? context : C.EXPRESSION,
                ((IObj) frm).meta()));
        else return ret;
      }
    ...
    static ObjExpr build(IPersistentVector interfaceSyms,
      ...
      if (thisSym != null) ret.thisName = thisSym.name;
      ...
      try {
        ...
        if (ret.isDeftype()) Var.popThreadBindings();
        Var.popThreadBindings();
      }
      ...
      for (ISeq s = RT.keys(ret.closes); s != null; s = s.next()) {
        ...
        int access = ACC_PUBLIC
            + (ret.isVolatile(lb) ? ACC_VOLATILE : ret.isMutable(lb) ? 0
                : ACC_FINAL);
        if (lb.getPrimitiveType() != null) cv.visitField(access, lb.name, Type
            .getType(lb.getPrimitiveType()).getDescriptor(), null, null);
        else
        // todo - when closed-overs are fields, use more specific types here and
        // in ctor and emitLocal?
    ...
    protected void emitStatics(ClassVisitor cv) {
      if (this.isDeftype()) {
        ...
        if (this.isDeftype() && this.fields.count() > this.hintedFields.count()) {
          ...
          if (hintedFields.count() > 0) for (i = 1; i <= fieldCount; i++) {
  ...
  public static class NewInstanceMethod extends ObjMethod {
    ...
    static NewInstanceMethod parse(ObjExpr objx, ISeq form, Symbol thistag,
      ...
      try {
        ...
        // register 'this' as local 0
        if (thisName != null) registerLocal((thisName == null) ? dummyThis
        ...
        for (int i = 0; i < parms.count(); i++) {
          ...
          Symbol p = (Symbol) parms.nth(i);
          Object tag = tagOf(p);
          if (tag != null) hinted = true;
          if (p.getNamespace() != null) p = Symbol.intern(p.name);
        ...
        if (matches.size() > 0) {
          ...
          if (matches.size() > 1) {
            ...
            if (!hinted) throw new IllegalArgumentException(
            ...
            if (m.getReturnType() != method.retClass) throw new IllegalArgumentException(
                "Mismatched return type: " + name.name + ", expected: "
        ...
        for (int i = 0; i < parms.count(); i++) {
          if (pclasses[i] == long.class || pclasses[i] == double.class) getAndIncLocalNum();
        }
        LOOP_LOCALS.set(argLocals);
    ...
    private static Map findMethodsWithNameAndArity(String name, int arity,
      ...
      for (Object o : mm.entrySet()) {
        Map.Entry e = (Map.Entry) o;
        java.lang.reflect.Method m = (java.lang.reflect.Method) e.getValue();
        if (name.equals(m.getName()) && m.getParameterTypes().length == arity) ret
    ...
    private static Map findMethodsWithName(String name, Map mm) {
      ...
      for (Object o : mm.entrySet()) {
        Map.Entry e = (Map.Entry) o;
        java.lang.reflect.Method m = (java.lang.reflect.Method) e.getValue();
        if (name.equals(m.getName())) ret.put(e.getKey(), e.getValue());
      }
      return ret;
    }
  ...
  static Class primClass(Symbol sym) {
    ...
    Class c = null;
    if (sym.name.equals("int")) c = int.class;
    else if (sym.name.equals("long")) c = long.class;
    else if (sym.name.equals("float")) c = float.class;
    else if (sym.name.equals("double")) c = double.class;
    else if (sym.name.equals("char")) c = char.class;
    else if (sym.name.equals("short")) c = short.class;
    else if (sym.name.equals("byte")) c = byte.class;
    else if (sym.name.equals("boolean")) c = boolean.class;
    else if (sym.name.equals("void")) c = void.class;
    return c;
  }

  static Class tagClass(Object tag) {
    if (tag == null) return Object.class;
    Class c = null;
    if (tag instanceof Symbol) c = primClass((Symbol) tag);
    if (c == null) c = HostExpr.tagToClass(tag);
    return c;
  }
  ...
  static Class boxClass(Class p) {
    ...
    if (p == Integer.TYPE) c = Integer.class;
    else if (p == Long.TYPE) c = Long.class;
    else if (p == Float.TYPE) c = Float.class;
    else if (p == Double.TYPE) c = Double.class;
    else if (p == Character.TYPE) c = Character.class;
    else if (p == Short.TYPE) c = Short.class;
    else if (p == Byte.TYPE) c = Byte.class;
    else if (p == Boolean.TYPE) c = Boolean.class;
  ...
  public static class CaseExpr implements Expr, MaybePrimitiveExpr {
    ...
    public CaseExpr(int line, int column, LocalBindingExpr expr, int shift,
      ...
      this.line = line;
      this.column = column;
      if (switchType != compactKey && switchType != sparseKey) throw new IllegalArgumentException(
          "Unexpected switch type: " + switchType);
      this.switchType = switchType;
      if (testType != intKey && testType != hashEquivKey
          && testType != hashIdentityKey) throw new IllegalArgumentException(
          "Unexpected test type: " + switchType);
    ...
    public void doEmit(C context, ObjExpr objx, GeneratorAdapter gen,
      ...
      if (testType == intKey) emitExprForInts(objx, gen, primExprType,
          defaultLabel);
      else emitExprForHashes(objx, gen);
      ...
      for (Integer i : labels.keySet()) {
        ...
        if (testType == intKey) emitThenForInts(objx, gen, primExprType,
            tests.get(i), thens.get(i), defaultLabel, emitUnboxed);
        else if (RT.contains(skipCheck, i) == RT.T) emitExpr(objx, gen,
            thens.get(i), emitUnboxed);
        else emitThenForHashes(objx, gen, tests.get(i), thens.get(i),
    ...
    private static void emitExpr(ObjExpr objx, GeneratorAdapter gen, Expr expr,
        boolean emitUnboxed) {
      if (emitUnboxed && expr instanceof MaybePrimitiveExpr) ((MaybePrimitiveExpr) expr)
          .emitUnboxed(C.EXPRESSION, objx, gen);
      else expr.emit(C.EXPRESSION, objx, gen);
    }
