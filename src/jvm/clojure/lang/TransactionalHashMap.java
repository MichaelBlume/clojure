...
public class TransactionalHashMap<K,V> extends AbstractMap<K,V> implements
    ConcurrentMap<K,V> {
  final Ref[] bins;
  ...
  public Set<Entry<K,V>> entrySet() {
    ...
    for (int i = 0; i < bins.length; i++) {
      ...
      if (map.count() > 0) entries.addAll((Collection) RT.seq(map));
    }
