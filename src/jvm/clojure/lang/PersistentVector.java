...
public class PersistentVector extends APersistentVector implements IObj,
    IEditableCollection {
  ...
  final static AtomicReference<Thread> NOEDIT = new AtomicReference<Thread>(
      null);
  ...
  public final static PersistentVector EMPTY = new PersistentVector(0, 5,
  ...
  PersistentVector(IPersistentMap meta, int cnt, int shift, Node root,
      Object[] tail) {
    this._meta = meta;
    this.cnt = cnt;
    this.shift = shift;
  ...
  public PersistentVector assocN(int i, Object val) {
    if (i >= 0 && i < cnt) {
      ...
      return new PersistentVector(meta(), cnt, shift, doAssoc(shift, root, i,
          val), tail);
    }
    if (i == cnt) return cons(val);
    throw new IndexOutOfBoundsException();
  }
  ...
  static public final class ChunkedSeq extends ASeq implements IChunkedSeq,
    ...
    ChunkedSeq(IPersistentMap meta, PersistentVector vec, Object[] node, int i,
        int offset) {
      super(meta);
      this.vec = vec;
      this.node = node;
    ...
    public ISeq chunkedNext() {
      if (i + node.length < vec.cnt) return new ChunkedSeq(vec,
          i + node.length, 0);
      return null;
    }
    ...
    public Obj withMeta(IPersistentMap meta) {
      if (meta == this._meta) return this;
    ...
    public ISeq next() {
      if (offset + 1 < node.length) return new ChunkedSeq(vec, node, i,
          offset + 1);
      return chunkedNext();
    }
  ...
  // private Node pushTail(int level, Node node, Object[] tailNode, Box
  // expansion){
  // Object newchild;
  // if(level == 0)
  // {
  ...
  // else
  // {
  // newchild = pushTail(level - 5, (Object[]) arr[arr.length - 1], tailNode,
  // expansion);
  ...
  public PersistentVector pop() {
    if (cnt == 0) throw new IllegalStateException("Can't pop empty vector");
    if (cnt == 1) return EMPTY.withMeta(meta());
    // if(tail.length > 1)
    if (cnt - tailoff() > 1) {
      Object[] newTail = new Object[tail.length - 1];
  ...
  static final class TransientVector extends AFn implements ITransientVector,
      Counted {
    int cnt;
    int shift;
    Node root;
    ...
    static Node editableRoot(Node node) {
      return new Node(new AtomicReference<Thread>(Thread.currentThread()),
          node.array.clone());
    }
    ...
    public Object invoke(Object arg1) {
      ...
      if (Util.isInteger(arg1)) return nth(((Number) arg1).intValue());
    ...
    private Node doAssoc(int level, Node node, int i, Object val) {
      ...
      if (level == 0) {
        ...
        int subidx = (i >>> level) & 0x01f;
        ret.array[subidx] = doAssoc(level - 5, (Node) node.array[subidx], i,
            val);
      }
      return ret;
    }
   ...
   * Integer.parseInt(args[1]); int reads = Integer.parseInt(args[2]); // Vector
   * v = new Vector(size); ArrayList v = new ArrayList(size); //
   ...
   * System.nanoTime() - startTime; System.out.println("time: " + estimatedTime
   * / 1000000); System.out.println("PersistentVector"); rand = new Random(42);
   * startTime = System.nanoTime(); long tp = 0;
   *
   ...
   * branching //oldp = oldp.assocN(rand2.nextInt(size), i); } for(int i = 0; i
   * < reads; i++) { // tp += (Integer) p.nth(rand.nextInt(size)); tp +=
   ...
   * size / 2; i++) { mp = mp.pop(); // p = p.pop(); v.remove(v.size() - 1); } p
   * = (PersistentVector) mp.immutable(); //mp.pop(); //should fail for(int i =
   * 0; i < size / 2; i++) { tp += (Integer) p.nth(i); tv += (Integer) v.get(i);
