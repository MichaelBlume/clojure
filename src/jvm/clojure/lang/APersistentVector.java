...
public abstract class APersistentVector extends AFn implements
    IPersistentVector, Iterable, List, RandomAccess, Comparable, Serializable,
    IHashEq {
  int _hash = -1;
  int _hasheq = -1;
  ...
  public ISeq seq() {
    if (count() > 0) return new Seq(this, 0);
    return null;
  }

  public ISeq rseq() {
    if (count() > 0) return new RSeq(this, count() - 1);
    return null;
  }
  ...
  static boolean doEquals(IPersistentVector v, Object obj) {
    ...
    if (obj instanceof List || obj instanceof IPersistentVector) {
      ...
      if (ma.size() != v.count() || ma.hashCode() != v.hashCode()) return false;
      for (Iterator i1 = ((List) v).iterator(), i2 = ma.iterator(); i1
          .hasNext();) {
        if (!Util.equals(i1.next(), i2.next())) return false;
      }
      return true;
    }
    ...
    // }
      ...
      for (int i = 0; i < v.count(); i++, ms = ms.next()) {
        if (ms == null || !Util.equals(v.nth(i), ms.first())) return false;
      }
  ...
  static boolean doEquiv(IPersistentVector v, Object obj) {
    if (obj instanceof List || obj instanceof IPersistentVector) {
      Collection ma = (Collection) obj;
      if (ma.size() != v.count()) return false;
      for (Iterator i1 = ((List) v).iterator(), i2 = ma.iterator(); i1
          .hasNext();) {
        if (!Util.equiv(i1.next(), i2.next())) return false;
      }
      return true;
    }
    // if(obj instanceof IPersistentVector)
    ...
    // }
      ...
      ISeq ms = RT.seq(obj);
      for (int i = 0; i < v.count(); i++, ms = ms.next()) {
        if (ms == null || !Util.equiv(v.nth(i), ms.first())) return false;
      }
      if (ms != null) return false;
    }
  ...
  public int indexOf(Object o) {
    for (int i = 0; i < count(); i++)
      if (Util.equiv(nth(i), o)) return i;
    return -1;
  }
  ...
  public IMapEntry entryAt(Object key) {
    if (Util.isInteger(key)) {
      int i = ((Number) key).intValue();
      if (i >= 0 && i < count()) return new MapEntry(key, nth(i));
    }
    return null;
  }
  ...
  public int compareTo(Object o) {
    ...
    else if (count() > v.count()) return 1;
    for (int i = 0; i < count(); i++) {
  ...
  static class Seq extends ASeq implements IndexedSeq, IReduce {
    ...
    public ISeq next() {
      if (i + 1 < v.count()) return new APersistentVector.Seq(v, i + 1);
      return null;
    }
  ...
  public static class RSeq extends ASeq implements IndexedSeq, Counted {
    ...
    public ISeq next() {
      if (i > 0) return new APersistentVector.RSeq(v, i - 1);
      return null;
    }
  ...
  static class SubVector extends APersistentVector implements IObj {
    ...
    public Iterator iterator() {
      return ((PersistentVector) v).rangedIterator(start, end);
    }

    public Object nth(int i) {
      if ((start + i >= end) || (i < 0)) throw new IndexOutOfBoundsException();
      return v.nth(start + i);
    }

    public IPersistentVector assocN(int i, Object val) {
      if (start + i > end) throw new IndexOutOfBoundsException();
      else if (start + i == end) return cons(val);
