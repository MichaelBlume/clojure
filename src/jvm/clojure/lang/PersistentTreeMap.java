...
public class PersistentTreeMap extends APersistentMap implements IObj,
  ...
  public PersistentTreeMap assoc(Object key, Object val) {
    ...
    if (t == null) // null == already contains key
    {
      ...
      if (foundNode.val() == val) // note only get same collection on identity
                                  // of val, not equals()
      return this;
  ...
  public ISeq seq() {
    if (_count > 0) return Seq.create(tree, true, _count);
    return null;
  }
  ...
  public ISeq rseq() {
    if (_count > 0) return Seq.create(tree, false, _count);
    return null;
  }
  ...
  public ISeq seq(boolean ascending) {
    if (_count > 0) return Seq.create(tree, ascending, _count);
    return null;
  }
  ...
  public ISeq seqFrom(Object key, boolean ascending) {
    if (_count > 0) {
      ...
      if (stack != null) return new Seq(stack, ascending);
    }
    return null;
  }
  ...
  public Object kvreduce(IFn f, Object init) {
    if (tree != null) init = tree.kvreduce(f, init);
    if (RT.isReduced(init)) init = ((IDeref) init).deref();
    return init;
  }
  ...
  Node add(Node t, Object key, Object val, Box found) {
    if (t == null) {
      if (val == null) return new Red(key);
  ...
  Node remove(Node t, Object key, Box found) {
    ...
    Node del = c < 0 ? remove(t.left(), key, found) : remove(t.right(), key,
        found);
    if (del == null && found.val == null) // not found below
    return null;
    if (c < 0) {
      if (t.left() instanceof Black) return balanceLeftDel(t.key, t.val(), del,
          t.right());
      else return red(t.key, t.val(), del, t.right());
    }
    if (t.right() instanceof Black) return balanceRightDel(t.key, t.val(),
        t.left(), del);
  ...
  static Node append(Node left, Node right) {
    if (left == null) return right;
      ...
      if (right instanceof Red) {
        ...
        if (app instanceof Red) return red(app.key, app.val(),
            red(left.key, left.val(), left.left(), app.left()),
            red(right.key, right.val(), app.right(), right.right()));
        else return red(left.key, left.val(), left.left(),
            red(right.key, right.val(), app, right.right()));
      } else return red(left.key, left.val(), left.left(),
          append(left.right(), right));
    } else if (right instanceof Red) return red(right.key, right.val(),
    ...
    else // black/black
    {
      ...
      else return balanceLeftDel(left.key, left.val(), left.left(),
          black(right.key, right.val(), app, right.right()));
    }
  }

  static Node balanceLeftDel(Object key, Object val, Node del, Node right) {
    if (del instanceof Red) return red(key, val, del.blacken(), right);
    else if (right instanceof Black) return rightBalance(key, val, del,
        right.redden());
    else if (right instanceof Red && right.left() instanceof Black) return red(
        right.left().key,
        right.left().val(),
        black(key, val, del, right.left().left()),
        rightBalance(right.key, right.val(), right.left().right(), right
            .right().redden()));
  ...
  static Node balanceRightDel(Object key, Object val, Node left, Node del) {
    if (del instanceof Red) return red(key, val, left, del.blacken());
    else if (left instanceof Black) return leftBalance(key, val, left.redden(),
        del);
    else if (left instanceof Red && left.right() instanceof Black) return red(
        left.right().key,
        left.right().val(),
        leftBalance(left.key, left.val(), left.left().redden(), left.right()
            .left()), black(key, val, left.right().right(), del));
    else throw new UnsupportedOperationException("Invariant violation");
  }

  static Node leftBalance(Object key, Object val, Node ins, Node right) {
    if (ins instanceof Red && ins.left() instanceof Red) return red(ins.key,
        ins.val(), ins.left().blacken(), black(key, val, ins.right(), right));
    else if (ins instanceof Red && ins.right() instanceof Red) return red(
        ins.right().key, ins.right().val(),
  ...
  static Node rightBalance(Object key, Object val, Node left, Node ins) {
    if (ins instanceof Red && ins.right() instanceof Red) return red(ins.key,
        ins.val(), black(key, val, left, ins.left()), ins.right().blacken());
    else if (ins instanceof Red && ins.left() instanceof Red) return red(
        ins.left().key, ins.left().val(),
  ...
  static class RedBranch extends Red {
    ...
    Node balanceLeft(Node parent) {
      if (left instanceof Red) return red(key, val(), left.blacken(),
    ...
    Node balanceRight(Node parent) {
      if (right instanceof Red) return red(key, val(),
          black(parent.key, parent.val(), parent.left(), left), right.blacken());
      else if (left instanceof Red) return red(left.key, left.val(),
   ...
   * // } // catch(Exception e) // { // e.printStackTrace(); //To change body of
   * catch statement use File | Settings | File Templates. // }
   * System.out.println("Building set"); //IPersistentMap set = new
   * PersistentArrayMap(); //IPersistentMap set = new
   * PersistentHashtableMap(1001); IPersistentMap set = PersistentHashMap.EMPTY;
   * //IPersistentMap set = new ListMap(); //IPersistentMap set = new
   * ArrayMap(); //IPersistentMap set = new PersistentTreeMap(); // for(int i =
   * 0; i < ints.length; i++) // { // Integer anInt = ints[i]; // set =
   * set.add(anInt); // } long startTime = System.nanoTime(); for(Integer anInt
   * : ints) { set = set.assoc(anInt, anInt); } //System.out.println("_count = "
   * + set.count());
   *
   * // System.out.println("_count = " + set._count + ", min: " + set.minKey() +
   * ", max: " + set.maxKey() // + ", depth: " + set.depth()); for(Object aSet :
   * set) { IMapEntry o = (IMapEntry) aSet; if(!set.contains(o.key()))
   ...
   * // Integer anInt = ints[i]; // ht.put(anInt,null); // } for(Integer anInt :
   * ints) { ht.put(anInt, anInt); } //System.out.println("size = " +
   ...
   * 1000000);
   *
   * System.out.println("set lookup"); startTime = System.nanoTime(); int c = 0;
   * for(Integer anInt : ints) { if(!set.contains(anInt)) ++c; } estimatedTime =
   ...
   * for(Integer anInt : ints) { if(!ht.containsKey(anInt)) ++c; } estimatedTime
   * = System.nanoTime() - startTime; System.out.println("notfound = " + c +
   * ", time: " + estimatedTime / 1000000);
   *
   * // System.out.println("_count = " + set._count + ", min: " + set.minKey() +
