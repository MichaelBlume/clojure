...
public class PersistentQueue extends Obj implements IPersistentList,
    Collection, Counted, IHashEq {

  final public static PersistentQueue EMPTY = new PersistentQueue(null, 0,
      null, null);
  ...
  public boolean equiv(Object obj) {
    ...
    for (ISeq s = seq(); s != null; s = s.next(), ms = ms.next()) {
      if (ms == null || !Util.equiv(s.first(), ms.first())) return false;
    }
    return ms == null;
  ...
  public PersistentQueue cons(Object o) {
    ...
    else return new PersistentQueue(meta(), cnt + 1, f, (r != null ? r
   ...
   *
   * Queue list = new LinkedList(); //Queue list = new ConcurrentLinkedQueue();
   * System.out.println("Queue"); startTime = System.nanoTime(); for(int i = 0;
   * i < n; i++) { list.add(i); list.add(i); list.remove(); } for(int i = 0; i <
   * n - 10; i++) { list.remove(); } estimatedTime = System.nanoTime() -
   * startTime; System.out.println("time: " + estimatedTime / 1000000);
   ...
   * // IPersistentList lastq = null; // IPersistentList lastq2; for(int i = 0;
   * i < n - 10; i++) { //lastq2 = lastq; //lastq = q; q = q.pop(); }
   * estimatedTime = System.nanoTime() - startTime; System.out.println("time: "
   * + estimatedTime / 1000000); System.out.println("peek: " + q.peek());
   *
   * IPersistentList q2 = q; for(int i = 0; i < 10; i++) { q2 =
   * (IPersistentList) q2.cons(i); } // for(ISeq s = q.seq();s != null;s =
   * s.rest()) // System.out.println("q: " + s.first().toString()); // for(ISeq
   * s = q2.seq();s != null;s = s.rest()) // System.out.println("q2: " +
   * s.first().toString()); }
   */
}
