...
public class Agent extends ARef {
  ...
  static class ActionQueue {
    ...
    static final ActionQueue EMPTY = new ActionQueue(PersistentQueue.EMPTY,
        null);
  ...
  AtomicReference<ActionQueue> aq = new AtomicReference<ActionQueue>(
      ActionQueue.EMPTY);
  ...
  volatile public static ExecutorService pooledExecutor = Executors
      .newFixedThreadPool(
          2 + Runtime.getRuntime().availableProcessors(),
          createThreadFactory("clojure-agent-send-pool-%d",
              sendThreadPoolCounter));

  volatile public static ExecutorService soloExecutor = Executors
  ...
  private static ThreadFactory createThreadFactory(final String format,
  ...
  static class Action implements Runnable {
    ...
    static void doRun(Action action) {
      try {
        ...
        if (error == null && next.q.count() > 0) ((Action) next.q.peek())
            .execute();
      } finally {
        nested.set(null);
      }
  ...
  synchronized public Object restart(Object newState, boolean clearActions) {
    ...
    if (clearActions) aq.set(ActionQueue.EMPTY);
    else {
      ...
      if (prior.q.count() > 0) ((Action) prior.q.peek()).execute();
    }
  ...
  void enqueue(Action action) {
    ...
    while (!queued) {
      prior = aq.get();
      queued = aq.compareAndSet(prior, new ActionQueue(
          (IPersistentStack) prior.q.cons(action), prior.error));
    }
