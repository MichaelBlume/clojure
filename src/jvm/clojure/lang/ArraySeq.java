...
public class ArraySeq extends ASeq implements IndexedSeq, IReduce {
  ...
  static ISeq createFromObject(Object array) {
    ...
    if (aclass == float[].class) return new ArraySeq_float(null,
        (float[]) array, 0);
    if (aclass == double[].class) return new ArraySeq_double(null,
        (double[]) array, 0);
    if (aclass == long[].class) return new ArraySeq_long(null, (long[]) array,
        0);
    if (aclass == byte[].class) return new ArraySeq_byte(null, (byte[]) array,
        0);
    if (aclass == char[].class) return new ArraySeq_char(null, (char[]) array,
        0);
    if (aclass == boolean[].class) return new ArraySeq_boolean(null,
        (boolean[]) array, 0);
    return new ArraySeq(array, 0);
  }
  ...
  public Object first() {
    if (oa != null) return oa[i];
    return Reflector.prepRet(ct, Array.get(array, i));
  }

  public ISeq next() {
    if (oa != null) {
      if (i + 1 < oa.length) return new ArraySeq(array, i + 1);
    } else {
      if (i + 1 < Array.getLength(array)) return new ArraySeq(array, i + 1);
    }
    return null;
  }

  public int count() {
    if (oa != null) return oa.length - i;
  ...
  public int indexOf(Object o) {
    if (oa != null) {
      ...
      for (int j = i; j < n; j++)
        if (Util.equals(o, Reflector.prepRet(ct, Array.get(array, j)))) return j
            - i;
    }
    return -1;
  }
  ...
  public int lastIndexOf(Object o) {
    if (oa != null) {
      ...
      if (o == null) {
        ...
        for (int j = Array.getLength(array) - 1; j >= i; j--)
          if (o.equals(Reflector.prepRet(ct, Array.get(array, j)))) return j
              - i;
      }
    }
    return -1;
  ...
  static public class ArraySeq_int extends ASeq implements IndexedSeq, IReduce {
    ...
    public ISeq next() {
      if (i + 1 < array.length) return new ArraySeq_int(meta(), array, i + 1);
      return null;
    }
  ...
  static public class ArraySeq_float extends ASeq implements IndexedSeq,
    ...
    public ISeq next() {
      if (i + 1 < array.length) return new ArraySeq_float(meta(), array, i + 1);
      return null;
    }
  ...
  static public class ArraySeq_double extends ASeq implements IndexedSeq,
    ...
    public ISeq next() {
      if (i + 1 < array.length) return new ArraySeq_double(meta(), array, i + 1);
      return null;
    }
  ...
  static public class ArraySeq_long extends ASeq implements IndexedSeq, IReduce {
    ...
    public ISeq next() {
      if (i + 1 < array.length) return new ArraySeq_long(meta(), array, i + 1);
      return null;
    }
  ...
  static public class ArraySeq_byte extends ASeq implements IndexedSeq, IReduce {
    ...
    public ISeq next() {
      if (i + 1 < array.length) return new ArraySeq_byte(meta(), array, i + 1);
      return null;
    }
  ...
  static public class ArraySeq_char extends ASeq implements IndexedSeq, IReduce {
    ...
    public ISeq next() {
      if (i + 1 < array.length) return new ArraySeq_char(meta(), array, i + 1);
      return null;
    }
  ...
  static public class ArraySeq_boolean extends ASeq implements IndexedSeq,
      IReduce {
    ...
    public ISeq next() {
      if (i + 1 < array.length) return new ArraySeq_boolean(meta(), array,
          i + 1);
      return null;
    }
