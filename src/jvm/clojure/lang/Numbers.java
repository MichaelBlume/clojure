...
public class Numbers {
  ...
  static BigInt toBigInt(Object x) {
    if (x instanceof BigInt) return (BigInt) x;
    if (x instanceof BigInteger) return BigInt.fromBigInteger((BigInteger) x);
    else return BigInt.fromLong(((Number) x).longValue());
  }

  static BigInteger toBigInteger(Object x) {
    if (x instanceof BigInteger) return (BigInteger) x;
    else if (x instanceof BigInt) return ((BigInt) x).toBigInteger();
  ...
  static BigDecimal toBigDecimal(Object x) {
    if (x instanceof BigDecimal) return (BigDecimal) x;
      ...
      BigInt bi = (BigInt) x;
      if (bi.bipart == null) return BigDecimal.valueOf(bi.lpart);
      else return new BigDecimal(bi.bipart);
    } else if (x instanceof BigInteger) return new BigDecimal((BigInteger) x);
    else if (x instanceof Double) return new BigDecimal(
        ((Number) x).doubleValue());
    else if (x instanceof Float) return new BigDecimal(
        ((Number) x).doubleValue());
  ...
  static public Ratio toRatio(Object x) {
    if (x instanceof Ratio) return (Ratio) x;
      ...
      int scale = bx.scale();
      if (scale < 0) return new Ratio(bv.multiply(BigInteger.TEN.pow(-scale)),
          BigInteger.ONE);
  ...
  static public Number rationalize(Number x) {
    if (x instanceof Float || x instanceof Double) return rationalize(BigDecimal
        .valueOf(x.doubleValue()));
    else if (x instanceof BigDecimal) {
      BigDecimal bx = (BigDecimal) x;
      BigInteger bv = bx.unscaledValue();
      int scale = bx.scale();
      if (scale < 0) return BigInt.fromBigInteger(bv.multiply(BigInteger.TEN
          .pow(-scale)));
  ...
  static public Number reduceBigInt(BigInt val) {
    if (val.bipart == null) return num(val.lpart);
    else return val.bipart;
  }

  static public Number divide(BigInteger n, BigInteger d) {
    if (d.equals(BigInteger.ZERO)) throw new ArithmeticException(
        "Divide by zero");
    BigInteger gcd = n.gcd(d);
    if (gcd.equals(BigInteger.ZERO)) return BigInt.ZERO;
    n = n.divide(gcd);
    d = d.divide(gcd);
    if (d.equals(BigInteger.ONE)) return BigInt.fromBigInteger(n);
    else if (d.equals(BigInteger.ONE.negate())) return BigInt.fromBigInteger(n
        .negate());
  ...
  final static class LongOps implements Ops {
    ...
    final public Number addP(Number x, Number y) {
      ...
      long ret = lx + ly;
      if ((ret ^ lx) < 0 && (ret ^ ly) < 0) return BIGINT_OPS.add(x, y);
      return num(ret);
    }
    ...
    final public Number multiplyP(Number x, Number y) {
      long lx = x.longValue(), ly = y.longValue();
      long ret = lx * ly;
      if (ly != 0 && ret / ly != lx) return BIGINT_OPS.multiply(x, y);
      return num(ret);
    }
    ...
    final public Number negateP(Number x) {
      long val = x.longValue();
      if (val > Long.MIN_VALUE) return num(-val);
    ...
    public Number incP(Number x) {
      long val = x.longValue();
      if (val < Long.MAX_VALUE) return num(val + 1);
    ...
    public Number decP(Number x) {
      long val = x.longValue();
      if (val > Long.MIN_VALUE) return num(val - 1);
  ...
  final static class RatioOps extends OpsP {
    ...
    static Number normalizeRet(Number ret, Number x, Number y) {
      // if(ret instanceof BigInteger && !(x instanceof BigInteger || y
    ...
    final public Number add(Number x, Number y) {
      ...
      Ratio ry = toRatio(y);
      Number ret = divide(
          ry.numerator.multiply(rx.denominator).add(
  ...
  final static class BigIntOps extends OpsP {
    ...
    public boolean isZero(Number x) {
      ...
      if (bx.bipart == null) return bx.lpart == 0;
  ...
  final static class BigDecimalOps extends OpsP {
    ...
    final public Number add(Number x, Number y) {
      ...
      return mc == null ? toBigDecimal(x).add(toBigDecimal(y))
    ...
    public Number divide(Number x, Number y) {
      MathContext mc = (MathContext) MATH_CONTEXT.deref();
      return mc == null ? toBigDecimal(x).divide(toBigDecimal(y))
          : toBigDecimal(x).divide(toBigDecimal(y), mc);
    }

    public Number quotient(Number x, Number y) {
      MathContext mc = (MathContext) MATH_CONTEXT.deref();
      return mc == null ? toBigDecimal(x)
          .divideToIntegralValue(toBigDecimal(y)) : toBigDecimal(x)
          .divideToIntegralValue(toBigDecimal(y), mc);
    }
    ...
    final public Number negate(Number x) {
      MathContext mc = (MathContext) MATH_CONTEXT.deref();
      return mc == null ? ((BigDecimal) x).negate() : ((BigDecimal) x)
          .negate(mc);
    }
    ...
    public Number dec(Number x) {
      MathContext mc = (MathContext) MATH_CONTEXT.deref();
      BigDecimal bx = (BigDecimal) x;
      return mc == null ? bx.subtract(BigDecimal.ONE) : bx.subtract(
          BigDecimal.ONE, mc);
    }
  }
  ...
  static Ops ops(Object x) {
    ...
    if (xc == Long.class) return LONG_OPS;
    else if (xc == Double.class) return DOUBLE_OPS;
    else if (xc == Integer.class) return LONG_OPS;
    else if (xc == Float.class) return DOUBLE_OPS;
    else if (xc == BigInt.class) return BIGINT_OPS;
    else if (xc == BigInteger.class) return BIGINT_OPS;
    else if (xc == Ratio.class) return RATIO_OPS;
    else if (xc == BigDecimal.class) return BIGDECIMAL_OPS;
    else return LONG_OPS;
  }

  static int hasheq(Number x) {
    Class xc = x.getClass();

    if (xc == Long.class || xc == Integer.class || xc == Short.class
        || xc == Byte.class) {
  ...
  static Category category(Object x) {
    ...
    if (xc == Integer.class) return Category.INTEGER;
    else if (xc == Double.class) return Category.FLOATING;
    else if (xc == Long.class) return Category.INTEGER;
    else if (xc == Float.class) return Category.FLOATING;
    else if (xc == BigInt.class) return Category.INTEGER;
    else if (xc == Ratio.class) return Category.RATIO;
    else if (xc == BigDecimal.class) return Category.DECIMAL;
    else return Category.INTEGER;
  }
  ...
  static long bitOpsCast(Object x) {
    ...
    if (xc == Long.class || xc == Integer.class || xc == Short.class
        || xc == Byte.class) return RT.longCast(x);
  ...
  static public float[] float_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new float[((Number) sizeOrSeq)
        .intValue()];
    else {
  ...
  static public double[] double_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new double[((Number) sizeOrSeq)
        .intValue()];
    else {
      ISeq s = RT.seq(sizeOrSeq);
      int size = RT.count(s);
  ...
  static public int[] int_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new int[((Number) sizeOrSeq)
        .intValue()];
    else {
      ISeq s = RT.seq(sizeOrSeq);
      int size = RT.count(s);
  ...
  static public long[] long_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new long[((Number) sizeOrSeq)
        .intValue()];
    else {
      ISeq s = RT.seq(sizeOrSeq);
      int size = RT.count(s);
  ...
  static public short[] short_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new short[((Number) sizeOrSeq)
        .intValue()];
    else {
      ISeq s = RT.seq(sizeOrSeq);
      int size = RT.count(s);
  ...
  static public char[] char_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new char[((Number) sizeOrSeq)
        .intValue()];
    else {
      ISeq s = RT.seq(sizeOrSeq);
      int size = RT.count(s);
  ...
  static public byte[] byte_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new byte[((Number) sizeOrSeq)
        .intValue()];
    else {
      ISeq s = RT.seq(sizeOrSeq);
      int size = RT.count(s);
  ...
  static public boolean[] boolean_array(Object sizeOrSeq) {
    if (sizeOrSeq instanceof Number) return new boolean[((Number) sizeOrSeq)
        .intValue()];
    else {
      ISeq s = RT.seq(sizeOrSeq);
      int size = RT.count(s);
  ...
  static public double unchecked_add(double x, double y) {
    return add(x, y);
  }

  static public double unchecked_minus(double x, double y) {
    return minus(x, y);
  }

  static public double unchecked_multiply(double x, double y) {
    return multiply(x, y);
  }

  static public double unchecked_minus(double x) {
    return minus(x);
  }

  static public double unchecked_inc(double x) {
    return inc(x);
  }

  static public double unchecked_dec(double x) {
    return dec(x);
  }

  static public double unchecked_add(double x, Object y) {
    return add(x, y);
  }

  static public double unchecked_minus(double x, Object y) {
    return minus(x, y);
  }

  static public double unchecked_multiply(double x, Object y) {
    return multiply(x, y);
  }

  static public double unchecked_add(Object x, double y) {
    return add(x, y);
  }

  static public double unchecked_minus(Object x, double y) {
    return minus(x, y);
  }

  static public double unchecked_multiply(Object x, double y) {
    return multiply(x, y);
  }

  static public double unchecked_add(double x, long y) {
    return add(x, y);
  }

  static public double unchecked_minus(double x, long y) {
    return minus(x, y);
  }

  static public double unchecked_multiply(double x, long y) {
    return multiply(x, y);
  }

  static public double unchecked_add(long x, double y) {
    return add(x, y);
  }

  static public double unchecked_minus(long x, double y) {
    return minus(x, y);
  }

  static public double unchecked_multiply(long x, double y) {
    return multiply(x, y);
  }

  static public Number unchecked_add(long x, Object y) {
    return add(x, y);
  }

  static public Number unchecked_minus(long x, Object y) {
    return minus(x, y);
  }

  static public Number unchecked_multiply(long x, Object y) {
    return multiply(x, y);
  }

  static public Number unchecked_add(Object x, long y) {
    return add(x, y);
  }

  static public Number unchecked_minus(Object x, long y) {
    return minus(x, y);
  }

  static public Number unchecked_multiply(Object x, long y) {
    return multiply(x, y);
  }

  static public Number quotient(double x, Object y) {
    return quotient((Object) x, y);
  }

  static public Number quotient(Object x, double y) {
    return quotient(x, (Object) y);
  }

  static public Number quotient(long x, Object y) {
    return quotient((Object) x, y);
  }

  static public Number quotient(Object x, long y) {
    return quotient(x, (Object) y);
  }

  static public double quotient(double x, long y) {
    return quotient(x, (double) y);
  }

  static public double quotient(long x, double y) {
    return quotient((double) x, y);
  }

  static public Number remainder(double x, Object y) {
    return remainder((Object) x, y);
  }

  static public Number remainder(Object x, double y) {
    return remainder(x, (Object) y);
  }

  static public Number remainder(long x, Object y) {
    return remainder((Object) x, y);
  }

  static public Number remainder(Object x, long y) {
    return remainder(x, (Object) y);
  }

  static public double remainder(double x, long y) {
    return remainder(x, (double) y);
  }

  static public double remainder(long x, double y) {
  ...
  static public Number minusP(long x) {
    if (x == Long.MIN_VALUE) return BigInt.fromBigInteger(BigInteger.valueOf(x)
        .negate());
    return num(-x);
  }
  ...
  static public Number incP(long x) {
    if (x == Long.MAX_VALUE) return BIGINT_OPS.inc(x);
    return num(x + 1);
  }
  ...
  static public Number decP(long x) {
    if (x == Long.MIN_VALUE) return BIGINT_OPS.dec(x);
    return num(x - 1);
  }
  ...
  /*
   * static public class F{ static public float add(float x, float y){ return x
   * + y; }
   *
   ...
   *
   * static public float aget(float[] xs, int i){ return xs[i]; }
   *
   * static public float aset(float[] xs, int i, float v){ xs[i] = v; return v;
   * }
   *
   * static public int alength(float[] xs){ return xs.length; }
   *
   * static public float[] aclone(float[] xs){ return xs.clone(); }
   *
   * static public float[] vec(int size, Object init){ float[] ret = new
   * float[size]; if(init instanceof Number) { float f = ((Number)
   * init).floatValue(); for(int i = 0; i < ret.length; i++) ret[i] = f; } else
   * { ISeq s = RT.seq(init); for(int i = 0; i < size && s != null; i++, s =
   * s.rest()) ret[i] = ((Number) s.first()).floatValue(); } return ret; }
   *
   * static public float[] vec(Object sizeOrSeq){ if(sizeOrSeq instanceof
   * Number) return new float[((Number) sizeOrSeq).intValue()]; else { ISeq s =
   * RT.seq(sizeOrSeq); int size = s.count(); float[] ret = new float[size];
   * for(int i = 0; i < size && s != null; i++, s = s.rest()) ret[i] = ((Number)
   * s.first()).intValue(); return ret; } }
   *
   *
   * static public float[] vsadd(float[] x, float y){ final float[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] += y; return xs; }
   *
   * static public float[] vssub(float[] x, float y){ final float[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] -= y; return xs; }
   *
   * static public float[] vsdiv(float[] x, float y){ final float[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] /= y; return xs; }
   *
   * static public float[] vsmul(float[] x, float y){ final float[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] *= y; return xs; }
   *
   * static public float[] svdiv(float y, float[] x){ final float[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = y / xs[i]; return xs;
   * }
   *
   * static public float[] vsmuladd(float[] x, float y, float[] zs){ final
   * float[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] *
   * y + zs[i]; return xs; }
   *
   * static public float[] vsmulsub(float[] x, float y, float[] zs){ final
   * float[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] *
   * y - zs[i]; return xs; }
   *
   * static public float[] vsmulsadd(float[] x, float y, float z){ final float[]
   * xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] * y + z;
   * return xs; }
   *
   * static public float[] vsmulssub(float[] x, float y, float z){ final float[]
   * xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] * y - z;
   * return xs; }
   *
   * static public float[] vabs(float[] x){ final float[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = Math.abs(xs[i]); return xs; }
   *
   * static public float[] vnegabs(float[] x){ final float[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = -Math.abs(xs[i]); return xs; }
   *
   * static public float[] vneg(float[] x){ final float[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = -xs[i]; return xs; }
   *
   * static public float[] vsqr(float[] x){ final float[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] *= xs[i]; return xs; }
   *
   * static public float[] vsignedsqr(float[] x){ final float[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] *= Math.abs(xs[i]); return xs; }
   *
   * static public float[] vclip(float[] x, float low, float high){ final
   * float[] xs = x.clone(); for(int i = 0; i < xs.length; i++) { if(xs[i] <
   * low) xs[i] = low; else if(xs[i] > high) xs[i] = high; } return xs; }
   *
   * static public IPersistentVector vclipcounts(float[] x, float low, float
   * high){ final float[] xs = x.clone(); int lowc = 0; int highc = 0;
   *
   * for(int i = 0; i < xs.length; i++) { if(xs[i] < low) { ++lowc; xs[i] = low;
   * } else if(xs[i] > high) { ++highc; xs[i] = high; } } return RT.vector(xs,
   * lowc, highc); }
   *
   ...
   *
   * static public float[] vreverse(float[] x){ final float[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = xs[xs.length - i - 1]; return
   * xs; }
   *
   * static public float[] vrunningsum(float[] x){ final float[] xs = x.clone();
   * for(int i = 1; i < xs.length; i++) xs[i] = xs[i - 1] + xs[i]; return xs; }
   *
   * static public float[] vsort(float[] x){ final float[] xs = x.clone();
   * Arrays.sort(xs); return xs; }
   *
   * static public float vdot(float[] xs, float[] ys){ float ret = 0; for(int i
   * = 0; i < xs.length; i++) ret += xs[i] * ys[i]; return ret; }
   *
   * static public float vmax(float[] xs){ if(xs.length == 0) return 0; float
   * ret = xs[0]; for(int i = 0; i < xs.length; i++) ret = Math.max(ret, xs[i]);
   * return ret; }
   *
   * static public float vmin(float[] xs){ if(xs.length == 0) return 0; float
   * ret = xs[0]; for(int i = 0; i < xs.length; i++) ret = Math.min(ret, xs[i]);
   * return ret; }
   *
   * static public float vmean(float[] xs){ if(xs.length == 0) return 0; return
   * vsum(xs) / xs.length; }
   *
   * static public double vrms(float[] xs){ if(xs.length == 0) return 0; float
   * ret = 0; for(int i = 0; i < xs.length; i++) ret += xs[i] * xs[i]; return
   * Math.sqrt(ret / xs.length); }
   *
   * static public float vsum(float[] xs){ float ret = 0; for(int i = 0; i <
   * xs.length; i++) ret += xs[i]; return ret; }
   *
   * static public boolean vequiv(float[] xs, float[] ys){ return
   * Arrays.equals(xs, ys); }
   *
   * static public float[] vadd(float[] x, float[] ys){ final float[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] += ys[i]; return xs; }
   *
   * static public float[] vsub(float[] x, float[] ys){ final float[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] -= ys[i]; return xs; }
   *
   * static public float[] vaddmul(float[] x, float[] ys, float[] zs){ final
   * float[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] +
   * ys[i]) * zs[i]; return xs; }
   *
   * static public float[] vsubmul(float[] x, float[] ys, float[] zs){ final
   * float[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] -
   * ys[i]) * zs[i]; return xs; }
   *
   * static public float[] vaddsmul(float[] x, float[] ys, float z){ final
   * float[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] +
   * ys[i]) * z; return xs; }
   *
   * static public float[] vsubsmul(float[] x, float[] ys, float z){ final
   * float[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] -
   * ys[i]) * z; return xs; }
   *
   * static public float[] vmulsadd(float[] x, float[] ys, float z){ final
   * float[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] *
   * ys[i]) + z; return xs; }
   *
   * static public float[] vdiv(float[] x, float[] ys){ final float[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] /= ys[i]; return xs; }
   *
   * static public float[] vmul(float[] x, float[] ys){ final float[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] *= ys[i]; return xs; }
   *
   * static public float[] vmuladd(float[] x, float[] ys, float[] zs){ final
   * float[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] *
   * ys[i]) + zs[i]; return xs; }
   *
   * static public float[] vmulsub(float[] x, float[] ys, float[] zs){ final
   * float[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] *
   * ys[i]) - zs[i]; return xs; }
   *
   * static public float[] vmax(float[] x, float[] ys){ final float[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = Math.max(xs[i],
   * ys[i]); return xs; }
   *
   * static public float[] vmin(float[] x, float[] ys){ final float[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = Math.min(xs[i],
   * ys[i]); return xs; }
   *
   * static public float[] vmap(IFn fn, float[] x) { float[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = ((Number)
   * fn.invoke(xs[i])).floatValue(); return xs; }
   *
   * static public float[] vmap(IFn fn, float[] x, float[] ys) { float[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = ((Number)
   * fn.invoke(xs[i], ys[i])).floatValue(); return xs; }
   *
   * }
   *
   * static public class D{ static public double add(double x, double y){ return
   * x + y; }
   *
   ...
   *
   * static public double aget(double[] xs, int i){ return xs[i]; }
   *
   * static public double aset(double[] xs, int i, double v){ xs[i] = v; return
   * v; }
   *
   * static public int alength(double[] xs){ return xs.length; }
   *
   * static public double[] aclone(double[] xs){ return xs.clone(); }
   *
   * static public double[] vec(int size, Object init){ double[] ret = new
   ...
   *
   * static public double[] vec(Object sizeOrSeq){ if(sizeOrSeq instanceof
   * Number) return new double[((Number) sizeOrSeq).intValue()]; else { ISeq s =
   * RT.seq(sizeOrSeq); int size = s.count(); double[] ret = new double[size];
   * for(int i = 0; i < size && s != null; i++, s = s.rest()) ret[i] = ((Number)
   * s.first()).intValue(); return ret; } }
   *
   * static public double[] vsadd(double[] x, double y){ final double[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] += y; return xs; }
   *
   * static public double[] vssub(double[] x, double y){ final double[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] -= y; return xs; }
   *
   * static public double[] vsdiv(double[] x, double y){ final double[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] /= y; return xs; }
   *
   * static public double[] vsmul(double[] x, double y){ final double[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] *= y; return xs; }
   *
   * static public double[] svdiv(double y, double[] x){ final double[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = y / xs[i]; return xs;
   * }
   *
   * static public double[] vsmuladd(double[] x, double y, double[] zs){ final
   * double[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] *
   * y + zs[i]; return xs; }
   *
   * static public double[] vsmulsub(double[] x, double y, double[] zs){ final
   * double[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] *
   * y - zs[i]; return xs; }
   *
   * static public double[] vsmulsadd(double[] x, double y, double z){ final
   * double[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] *
   * y + z; return xs; }
   *
   * static public double[] vsmulssub(double[] x, double y, double z){ final
   * double[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] *
   * y - z; return xs; }
   *
   * static public double[] vabs(double[] x){ final double[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = Math.abs(xs[i]); return xs; }
   *
   * static public double[] vnegabs(double[] x){ final double[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = -Math.abs(xs[i]); return xs; }
   *
   * static public double[] vneg(double[] x){ final double[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = -xs[i]; return xs; }
   *
   * static public double[] vsqr(double[] x){ final double[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] *= xs[i]; return xs; }
   *
   * static public double[] vsignedsqr(double[] x){ final double[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] *= Math.abs(xs[i]);
   * return xs; }
   *
   * static public double[] vclip(double[] x, double low, double high){ final
   * double[] xs = x.clone(); for(int i = 0; i < xs.length; i++) { if(xs[i] <
   * low) xs[i] = low; else if(xs[i] > high) xs[i] = high; } return xs; }
   *
   * static public IPersistentVector vclipcounts(double[] x, double low, double
   ...
   * lowc, highc); }
   *
   * static public double[] vthresh(double[] x, double thresh, double
   * otherwise){ final double[] xs = x.clone(); for(int i = 0; i < xs.length;
   * i++) { if(xs[i] < thresh) xs[i] = otherwise; } return xs; }
   *
   * static public double[] vreverse(double[] x){ final double[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = xs[xs.length - i - 1]; return
   * xs; }
   *
   * static public double[] vrunningsum(double[] x){ final double[] xs =
   * x.clone(); for(int i = 1; i < xs.length; i++) xs[i] = xs[i - 1] + xs[i];
   * return xs; }
   *
   * static public double[] vsort(double[] x){ final double[] xs = x.clone();
   * Arrays.sort(xs); return xs; }
   *
   * static public double vdot(double[] xs, double[] ys){ double ret = 0;
   * for(int i = 0; i < xs.length; i++) ret += xs[i] * ys[i]; return ret; }
   *
   * static public double vmax(double[] xs){ if(xs.length == 0) return 0; double
   * ret = xs[0]; for(int i = 0; i < xs.length; i++) ret = Math.max(ret, xs[i]);
   * return ret; }
   *
   * static public double vmin(double[] xs){ if(xs.length == 0) return 0; double
   * ret = xs[0]; for(int i = 0; i < xs.length; i++) ret = Math.min(ret, xs[i]);
   * return ret; }
   *
   * static public double vmean(double[] xs){ if(xs.length == 0) return 0;
   * return vsum(xs) / xs.length; }
   *
   * static public double vrms(double[] xs){ if(xs.length == 0) return 0; double
   * ret = 0; for(int i = 0; i < xs.length; i++) ret += xs[i] * xs[i]; return
   * Math.sqrt(ret / xs.length); }
   *
   * static public double vsum(double[] xs){ double ret = 0; for(int i = 0; i <
   * xs.length; i++) ret += xs[i]; return ret; }
   *
   * static public boolean vequiv(double[] xs, double[] ys){ return
   * Arrays.equals(xs, ys); }
   *
   * static public double[] vadd(double[] x, double[] ys){ final double[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] += ys[i]; return xs; }
   *
   * static public double[] vsub(double[] x, double[] ys){ final double[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] -= ys[i]; return xs; }
   *
   * static public double[] vaddmul(double[] x, double[] ys, double[] zs){ final
   * double[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i]
   * + ys[i]) * zs[i]; return xs; }
   *
   * static public double[] vsubmul(double[] x, double[] ys, double[] zs){ final
   * double[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i]
   * - ys[i]) * zs[i]; return xs; }
   *
   * static public double[] vaddsmul(double[] x, double[] ys, double z){ final
   * double[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i]
   * + ys[i]) * z; return xs; }
   *
   * static public double[] vsubsmul(double[] x, double[] ys, double z){ final
   * double[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i]
   * - ys[i]) * z; return xs; }
   *
   * static public double[] vmulsadd(double[] x, double[] ys, double z){ final
   * double[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i]
   * * ys[i]) + z; return xs; }
   *
   * static public double[] vdiv(double[] x, double[] ys){ final double[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] /= ys[i]; return xs; }
   *
   * static public double[] vmul(double[] x, double[] ys){ final double[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] *= ys[i]; return xs; }
   *
   * static public double[] vmuladd(double[] x, double[] ys, double[] zs){ final
   * double[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i]
   * * ys[i]) + zs[i]; return xs; }
   *
   * static public double[] vmulsub(double[] x, double[] ys, double[] zs){ final
   * double[] xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i]
   * * ys[i]) - zs[i]; return xs; }
   *
   * static public double[] vmax(double[] x, double[] ys){ final double[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = Math.max(xs[i],
   * ys[i]); return xs; }
   *
   * static public double[] vmin(double[] x, double[] ys){ final double[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = Math.min(xs[i],
   * ys[i]); return xs; }
   *
   * static public double[] vmap(IFn fn, double[] x) { double[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = ((Number)
   * fn.invoke(xs[i])).doubleValue(); return xs; }
   *
   * static public double[] vmap(IFn fn, double[] x, double[] ys) { double[] xs
   * = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = ((Number)
   * fn.invoke(xs[i], ys[i])).doubleValue(); return xs; } }
   *
   * static public class I{ static public int add(int x, int y){ return x + y; }
   *
   ...
   *
   * static public int aget(int[] xs, int i){ return xs[i]; }
   *
   * static public int aset(int[] xs, int i, int v){ xs[i] = v; return v; }
   *
   * static public int alength(int[] xs){ return xs.length; }
   *
   * static public int[] aclone(int[] xs){ return xs.clone(); }
   *
   * static public int[] vec(int size, Object init){ int[] ret = new int[size];
   * if(init instanceof Number) { int f = ((Number) init).intValue(); for(int i
   * = 0; i < ret.length; i++) ret[i] = f; } else { ISeq s = RT.seq(init);
   * for(int i = 0; i < size && s != null; i++, s = s.rest()) ret[i] = ((Number)
   * s.first()).intValue(); } return ret; }
   *
   * static public int[] vec(Object sizeOrSeq){ if(sizeOrSeq instanceof Number)
   * return new int[((Number) sizeOrSeq).intValue()]; else { ISeq s =
   * RT.seq(sizeOrSeq); int size = s.count(); int[] ret = new int[size]; for(int
   * i = 0; i < size && s != null; i++, s = s.rest()) ret[i] = ((Number)
   * s.first()).intValue(); return ret; } }
   *
   * static public int[] vsadd(int[] x, int y){ final int[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] += y; return xs; }
   *
   * static public int[] vssub(int[] x, int y){ final int[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] -= y; return xs; }
   *
   * static public int[] vsdiv(int[] x, int y){ final int[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] /= y; return xs; }
   *
   * static public int[] vsmul(int[] x, int y){ final int[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] *= y; return xs; }
   *
   * static public int[] svdiv(int y, int[] x){ final int[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = y / xs[i]; return xs; }
   *
   * static public int[] vsmuladd(int[] x, int y, int[] zs){ final int[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] * y + zs[i];
   * return xs; }
   *
   * static public int[] vsmulsub(int[] x, int y, int[] zs){ final int[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] * y - zs[i];
   * return xs; }
   *
   * static public int[] vsmulsadd(int[] x, int y, int z){ final int[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] * y + z; return
   * xs; }
   *
   * static public int[] vsmulssub(int[] x, int y, int z){ final int[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] * y - z; return
   * xs; }
   *
   * static public int[] vabs(int[] x){ final int[] xs = x.clone(); for(int i =
   * 0; i < xs.length; i++) xs[i] = Math.abs(xs[i]); return xs; }
   *
   * static public int[] vnegabs(int[] x){ final int[] xs = x.clone(); for(int i
   * = 0; i < xs.length; i++) xs[i] = -Math.abs(xs[i]); return xs; }
   *
   * static public int[] vneg(int[] x){ final int[] xs = x.clone(); for(int i =
   * 0; i < xs.length; i++) xs[i] = -xs[i]; return xs; }
   *
   * static public int[] vsqr(int[] x){ final int[] xs = x.clone(); for(int i =
   * 0; i < xs.length; i++) xs[i] *= xs[i]; return xs; }
   *
   * static public int[] vsignedsqr(int[] x){ final int[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] *= Math.abs(xs[i]); return xs; }
   *
   * static public int[] vclip(int[] x, int low, int high){ final int[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) { if(xs[i] < low) xs[i] =
   ...
   * lowc, highc); }
   *
   * static public int[] vthresh(int[] x, int thresh, int otherwise){ final
   * int[] xs = x.clone(); for(int i = 0; i < xs.length; i++) { if(xs[i] <
   * thresh) xs[i] = otherwise; } return xs; }
   *
   * static public int[] vreverse(int[] x){ final int[] xs = x.clone(); for(int
   * i = 0; i < xs.length; i++) xs[i] = xs[xs.length - i - 1]; return xs; }
   *
   * static public int[] vrunningsum(int[] x){ final int[] xs = x.clone();
   * for(int i = 1; i < xs.length; i++) xs[i] = xs[i - 1] + xs[i]; return xs; }
   *
   * static public int[] vsort(int[] x){ final int[] xs = x.clone();
   * Arrays.sort(xs); return xs; }
   *
   * static public int vdot(int[] xs, int[] ys){ int ret = 0; for(int i = 0; i <
   * xs.length; i++) ret += xs[i] * ys[i]; return ret; }
   *
   * static public int vmax(int[] xs){ if(xs.length == 0) return 0; int ret =
   * xs[0]; for(int i = 0; i < xs.length; i++) ret = Math.max(ret, xs[i]);
   * return ret; }
   *
   * static public int vmin(int[] xs){ if(xs.length == 0) return 0; int ret =
   * xs[0]; for(int i = 0; i < xs.length; i++) ret = Math.min(ret, xs[i]);
   * return ret; }
   *
   * static public double vmean(int[] xs){ if(xs.length == 0) return 0; return
   * vsum(xs) / (double) xs.length; }
   *
   * static public double vrms(int[] xs){ if(xs.length == 0) return 0; int ret =
   * 0; for(int i = 0; i < xs.length; i++) ret += xs[i] * xs[i]; return
   * Math.sqrt(ret / (double) xs.length); }
   *
   * static public int vsum(int[] xs){ int ret = 0; for(int i = 0; i <
   * xs.length; i++) ret += xs[i]; return ret; }
   *
   * static public boolean vequiv(int[] xs, int[] ys){ return Arrays.equals(xs,
   * ys); }
   *
   * static public int[] vadd(int[] x, int[] ys){ final int[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] += ys[i]; return xs; }
   *
   * static public int[] vsub(int[] x, int[] ys){ final int[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] -= ys[i]; return xs; }
   *
   * static public int[] vaddmul(int[] x, int[] ys, int[] zs){ final int[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] + ys[i]) *
   * zs[i]; return xs; }
   *
   * static public int[] vsubmul(int[] x, int[] ys, int[] zs){ final int[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] - ys[i]) *
   * zs[i]; return xs; }
   *
   * static public int[] vaddsmul(int[] x, int[] ys, int z){ final int[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] + ys[i]) * z;
   * return xs; }
   *
   * static public int[] vsubsmul(int[] x, int[] ys, int z){ final int[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] - ys[i]) * z;
   * return xs; }
   *
   * static public int[] vmulsadd(int[] x, int[] ys, int z){ final int[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] * ys[i]) + z;
   * return xs; }
   *
   * static public int[] vdiv(int[] x, int[] ys){ final int[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] /= ys[i]; return xs; }
   *
   * static public int[] vmul(int[] x, int[] ys){ final int[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] *= ys[i]; return xs; }
   *
   * static public int[] vmuladd(int[] x, int[] ys, int[] zs){ final int[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] * ys[i]) +
   * zs[i]; return xs; }
   *
   * static public int[] vmulsub(int[] x, int[] ys, int[] zs){ final int[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] * ys[i]) -
   * zs[i]; return xs; }
   *
   * static public int[] vmax(int[] x, int[] ys){ final int[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = Math.max(xs[i], ys[i]); return
   * xs; }
   *
   * static public int[] vmin(int[] x, int[] ys){ final int[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = Math.min(xs[i], ys[i]); return
   * xs; }
   *
   * static public int[] vmap(IFn fn, int[] x) { int[] xs = x.clone(); for(int i
   * = 0; i < xs.length; i++) xs[i] = ((Number) fn.invoke(xs[i])).intValue();
   * return xs; }
   *
   * static public int[] vmap(IFn fn, int[] x, int[] ys) { int[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = ((Number) fn.invoke(xs[i],
   * ys[i])).intValue(); return xs; }
   *
   * }
   *
   * static public class L{ static public long add(long x, long y){ return x +
   * y; }
   *
   ...
   *
   * static public long aget(long[] xs, int i){ return xs[i]; }
   *
   * static public long aset(long[] xs, int i, long v){ xs[i] = v; return v; }
   *
   * static public int alength(long[] xs){ return xs.length; }
   *
   * static public long[] aclone(long[] xs){ return xs.clone(); }
   *
   * static public long[] vec(int size, Object init){ long[] ret = new
   * long[size]; if(init instanceof Number) { long f = ((Number)
   * init).longValue(); for(int i = 0; i < ret.length; i++) ret[i] = f; } else {
   * ISeq s = RT.seq(init); for(int i = 0; i < size && s != null; i++, s =
   * s.rest()) ret[i] = ((Number) s.first()).longValue(); } return ret; }
   *
   * static public long[] vec(Object sizeOrSeq){ if(sizeOrSeq instanceof Number)
   * return new long[((Number) sizeOrSeq).intValue()]; else { ISeq s =
   * RT.seq(sizeOrSeq); int size = s.count(); long[] ret = new long[size];
   * for(int i = 0; i < size && s != null; i++, s = s.rest()) ret[i] = ((Number)
   * s.first()).intValue(); return ret; } }
   *
   *
   * static public long[] vsadd(long[] x, long y){ final long[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] += y; return xs; }
   *
   * static public long[] vssub(long[] x, long y){ final long[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] -= y; return xs; }
   *
   * static public long[] vsdiv(long[] x, long y){ final long[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] /= y; return xs; }
   *
   * static public long[] vsmul(long[] x, long y){ final long[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] *= y; return xs; }
   *
   * static public long[] svdiv(long y, long[] x){ final long[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = y / xs[i]; return xs; }
   *
   * static public long[] vsmuladd(long[] x, long y, long[] zs){ final long[] xs
   * = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] * y + zs[i];
   * return xs; }
   *
   * static public long[] vsmulsub(long[] x, long y, long[] zs){ final long[] xs
   * = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] * y - zs[i];
   * return xs; }
   *
   * static public long[] vsmulsadd(long[] x, long y, long z){ final long[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] * y + z; return
   * xs; }
   *
   * static public long[] vsmulssub(long[] x, long y, long z){ final long[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = xs[i] * y - z; return
   * xs; }
   *
   * static public long[] vabs(long[] x){ final long[] xs = x.clone(); for(int i
   * = 0; i < xs.length; i++) xs[i] = Math.abs(xs[i]); return xs; }
   *
   * static public long[] vnegabs(long[] x){ final long[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = -Math.abs(xs[i]); return xs; }
   *
   * static public long[] vneg(long[] x){ final long[] xs = x.clone(); for(int i
   * = 0; i < xs.length; i++) xs[i] = -xs[i]; return xs; }
   *
   * static public long[] vsqr(long[] x){ final long[] xs = x.clone(); for(int i
   * = 0; i < xs.length; i++) xs[i] *= xs[i]; return xs; }
   *
   * static public long[] vsignedsqr(long[] x){ final long[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] *= Math.abs(xs[i]); return xs; }
   *
   * static public long[] vclip(long[] x, long low, long high){ final long[] xs
   * = x.clone(); for(int i = 0; i < xs.length; i++) { if(xs[i] < low) xs[i] =
   * low; else if(xs[i] > high) xs[i] = high; } return xs; }
   *
   ...
   * lowc, highc); }
   *
   * static public long[] vthresh(long[] x, long thresh, long otherwise){ final
   * long[] xs = x.clone(); for(int i = 0; i < xs.length; i++) { if(xs[i] <
   * thresh) xs[i] = otherwise; } return xs; }
   *
   * static public long[] vreverse(long[] x){ final long[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = xs[xs.length - i - 1]; return
   * xs; }
   *
   * static public long[] vrunningsum(long[] x){ final long[] xs = x.clone();
   * for(int i = 1; i < xs.length; i++) xs[i] = xs[i - 1] + xs[i]; return xs; }
   *
   * static public long[] vsort(long[] x){ final long[] xs = x.clone();
   * Arrays.sort(xs); return xs; }
   *
   * static public long vdot(long[] xs, long[] ys){ long ret = 0; for(int i = 0;
   * i < xs.length; i++) ret += xs[i] * ys[i]; return ret; }
   *
   * static public long vmax(long[] xs){ if(xs.length == 0) return 0; long ret =
   * xs[0]; for(int i = 0; i < xs.length; i++) ret = Math.max(ret, xs[i]);
   * return ret; }
   *
   * static public long vmin(long[] xs){ if(xs.length == 0) return 0; long ret =
   * xs[0]; for(int i = 0; i < xs.length; i++) ret = Math.min(ret, xs[i]);
   * return ret; }
   *
   * static public double vmean(long[] xs){ if(xs.length == 0) return 0; return
   * vsum(xs) / (double) xs.length; }
   *
   * static public double vrms(long[] xs){ if(xs.length == 0) return 0; long ret
   * = 0; for(int i = 0; i < xs.length; i++) ret += xs[i] * xs[i]; return
   * Math.sqrt(ret / (double) xs.length); }
   *
   * static public long vsum(long[] xs){ long ret = 0; for(int i = 0; i <
   * xs.length; i++) ret += xs[i]; return ret; }
   *
   * static public boolean vequiv(long[] xs, long[] ys){ return
   * Arrays.equals(xs, ys); }
   *
   * static public long[] vadd(long[] x, long[] ys){ final long[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] += ys[i]; return xs; }
   *
   * static public long[] vsub(long[] x, long[] ys){ final long[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] -= ys[i]; return xs; }
   *
   * static public long[] vaddmul(long[] x, long[] ys, long[] zs){ final long[]
   * xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] + ys[i])
   * * zs[i]; return xs; }
   *
   * static public long[] vsubmul(long[] x, long[] ys, long[] zs){ final long[]
   * xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] - ys[i])
   * * zs[i]; return xs; }
   *
   * static public long[] vaddsmul(long[] x, long[] ys, long z){ final long[] xs
   * = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] + ys[i]) *
   * z; return xs; }
   *
   * static public long[] vsubsmul(long[] x, long[] ys, long z){ final long[] xs
   * = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] - ys[i]) *
   * z; return xs; }
   *
   * static public long[] vmulsadd(long[] x, long[] ys, long z){ final long[] xs
   * = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] * ys[i]) +
   * z; return xs; }
   *
   * static public long[] vdiv(long[] x, long[] ys){ final long[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] /= ys[i]; return xs; }
   *
   * static public long[] vmul(long[] x, long[] ys){ final long[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] *= ys[i]; return xs; }
   *
   * static public long[] vmuladd(long[] x, long[] ys, long[] zs){ final long[]
   * xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] * ys[i])
   * + zs[i]; return xs; }
   *
   * static public long[] vmulsub(long[] x, long[] ys, long[] zs){ final long[]
   * xs = x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = (xs[i] * ys[i])
   * - zs[i]; return xs; }
   *
   * static public long[] vmax(long[] x, long[] ys){ final long[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = Math.max(xs[i],
   * ys[i]); return xs; }
   *
   * static public long[] vmin(long[] x, long[] ys){ final long[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = Math.min(xs[i],
   * ys[i]); return xs; }
   *
   * static public long[] vmap(IFn fn, long[] x) { long[] xs = x.clone();
   * for(int i = 0; i < xs.length; i++) xs[i] = ((Number)
   * fn.invoke(xs[i])).longValue(); return xs; }
   *
   * static public long[] vmap(IFn fn, long[] x, long[] ys) { long[] xs =
   * x.clone(); for(int i = 0; i < xs.length; i++) xs[i] = ((Number)
