...
public class PersistentStructMap extends APersistentMap implements IObj {
  ...
  static public Def createSlotMap(ISeq keys) {
    if (keys == null) throw new IllegalArgumentException("Must supply keys");
    int c = RT.count(keys);
  ...
  static public PersistentStructMap create(Def def, ISeq keyvals) {
    ...
    for (; keyvals != null; keyvals = keyvals.next().next()) {
      if (keyvals.next() == null) throw new IllegalArgumentException(
          String.format("No value supplied for key: %s", keyvals.first()));
      Object k = keyvals.first();
      Object v = RT.second(keyvals);
      Map.Entry e = def.keyslots.entryAt(k);
      if (e != null) vals[(Integer) e.getValue()] = v;
      else ext = ext.assoc(k, v);
    }
  ...
  static public PersistentStructMap construct(Def def, ISeq valseq) {
    ...
    if (valseq != null) throw new IllegalArgumentException(
    ...
    if (e != null) {
      ...
      return new AFn() {
        public Object invoke(Object arg1) {
          ...
          if (m.def != def) throw Util
              .runtimeException("Accessor/struct mismatch");
          return m.vals[i];
        }
      };
    }
  ...
  protected PersistentStructMap(IPersistentMap meta, Def def, Object[] vals,
      IPersistentMap ext) {
    this._meta = meta;
    this.ext = ext;
    this.def = def;
   ...
   * This function is used instead of the PersistentStructMap constructor by all
   * methods that return a new PersistentStructMap. This is done so as to allow
   * subclasses to return instances of their class from all PersistentStructMap
   * methods.
   */
  protected PersistentStructMap makeNew(IPersistentMap meta, Def def,
  ...
  public IPersistentMap without(Object key) {
    Map.Entry e = def.keyslots.entryAt(key);
    if (e != null) throw Util.runtimeException("Can't remove struct key");
    IPersistentMap newExt = ext.without(key);
    if (newExt == ext) return this;
  ...
  static class Seq extends ASeq {
    ...
    public Seq(IPersistentMap meta, ISeq keys, Object[] vals, int i,
        IPersistentMap ext) {
      super(meta);
      this.i = i;
      this.keys = keys;
    ...
    public Obj withMeta(IPersistentMap meta) {
      if (meta != _meta) return new Seq(meta, keys, vals, i, ext);
      return this;
    }
    ...
    public ISeq next() {
      if (i + 1 < vals.length) return new Seq(_meta, keys.next(), vals, i + 1,
          ext);
      return ext.seq();
    }
  }
