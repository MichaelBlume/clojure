...
public class LispReader {
  ...
  static Symbol UNQUOTE_SPLICING = Symbol.intern("clojure.core",
      "unquote-splicing");
  ...
  // Pattern.compile("[:]?([\\D&&[^:/]][^:/]*/)?[\\D&&[^:/]][^:/]*");
  static Pattern symbolPat = Pattern
      .compile("[:]?([\\D&&[^/]].*/)?([\\D&&[^/]][^/]*)");
  // static Pattern varPat =
  // Pattern.compile("([\\D&&[^:\\.]][^:\\.]*):([\\D&&[^:\\.]][^:\\.]*)");
  // static Pattern intPat = Pattern.compile("[-+]?[0-9]+\\.?");
  static Pattern intPat = Pattern
      .compile("([-+]?)(?:(0)|([1-9][0-9]*)|0[xX]([0-9A-Fa-f]+)|0([0-7]+)|([1-9][0-9]?)[rR]([0-9A-Za-z]+)|0[0-9]+)(N)?");
  static Pattern ratioPat = Pattern.compile("([-+]?[0-9]+)/([0-9]+)");
  static Pattern floatPat = Pattern
      .compile("([-+]?[0-9]+(\\.[0-9]*)?([eE][-+]?[0-9]+)?)(M)?");
  ...
  static public Object read(PushbackReader r, boolean eofIsError,
      Object eofValue, boolean isRecursive) {
    ...
    try {
      ...
      if (isRecursive || !(r instanceof LineNumberingPushbackReader)) throw Util
          .sneakyThrow(e);
      LineNumberingPushbackReader rdr = (LineNumberingPushbackReader) r;
      // throw Util.runtimeException(String.format("ReaderError:(%d,1) %s",
  ...
  static private int readUnicodeChar(String token, int offset, int length,
      int base) {
    if (token.length() != offset + length) throw new IllegalArgumentException(
        "Invalid unicode character: \\" + token);
    int uc = 0;
    for (int i = offset; i < offset + length; ++i) {
      int d = Character.digit(token.charAt(i), base);
      if (d == -1) throw new IllegalArgumentException("Invalid digit: "
          + token.charAt(i));
      uc = uc * base + d;
    }
    return (char) uc;
  ...
  static private int readUnicodeChar(PushbackReader r, int initch, int base,
      int length, boolean exact) {
    int uc = Character.digit(initch, base);
    if (uc == -1) throw new IllegalArgumentException("Invalid digit: "
        + (char) initch);
    int i = 1;
    for (; i < length; ++i) {
      int ch = read1(r);
    ...
    if (i != length && exact) throw new IllegalArgumentException(
  ...
  private static Object matchSymbol(String s) {
    ...
    if (m.matches()) {
      ...
      if (ns != null && ns.endsWith(":/") || name.endsWith(":")
          || s.indexOf("::", 1) != -1) return null;
      if (s.startsWith("::")) {
        Symbol ks = Symbol.intern(s.substring(2));
        Namespace kns;
        if (ks.ns != null) kns = Compiler.namespaceFor(ks);
        else kns = Compiler.currentNS();
        // auto-resolving keyword
        if (kns != null) return Keyword.intern(kns.name.name, ks.name);
        else return null;
      }
  ...
  private static Object matchNumber(String s) {
    ...
    if (m.matches()) {
      ...
      return Numbers.divide(Numbers.reduceBigInt(BigInt
          .fromBigInteger(new BigInteger(numerator))), Numbers
      ...
      for (int ch = read1(r); ch != '"'; ch = read1(r)) {
        ...
        if (ch == '\\') // escape
        {
          ...
          switch (ch) {
            ...
            case 'u': {
              ch = read1(r);
              if (Character.digit(ch, 16) == -1) throw Util
                  .runtimeException("Invalid unicode escape: \\u" + (char) ch);
  ...
  public static class DeprecatedWrappingReader extends AFn {
    ...
    public Object invoke(Object reader, Object quote) {
      System.out.println("WARNING: reader macro " + macro
          + " is deprecated; use " + sym.getName() + " instead");
      PushbackReader r = (PushbackReader) reader;
      Object o = read(r, true, null, true);
      return RT.list(sym, o);
    }
   ...
   *
   * public Object invoke(Object reader, Object quote) { PushbackReader r =
   * (PushbackReader) reader; int ch = read1(r); if(ch == -1) throw
   * Util.runtimeException("EOF while reading character"); if(ch == '!') {
   * Object o = read(r, true, null, true); return RT.list(DEREF_BANG, o); } else
   * { r.unread(ch); Object o = read(r, true, null, true); return RT.list(DEREF,
   * o); } }
   *
   * }
   */
  ...
  public static class DispatchReader extends AFn {
    public Object invoke(Object reader, Object hash) {
      ...
      if (fn == null) {
        ...
        else throw Util.runtimeException(String.format(
            "No dispatch macro for: %c", (char) ch));
      }
  ...
  public static class FnReader extends AFn {
    public Object invoke(Object reader, Object lparen) {
      PushbackReader r = (PushbackReader) reader;
      if (ARG_ENV.deref() != null) throw new IllegalStateException(
  ...
  static class ArgReader extends AFn {
    public Object invoke(Object reader, Object pct) {
      ...
      if (n.equals(Compiler._AMP_)) return registerArg(-1);
      if (!(n instanceof Number)) throw new IllegalStateException(
  ...
  public static class MetaReader extends AFn {
    public Object invoke(Object reader, Object caret) {
      ...
      if (meta instanceof Symbol || meta instanceof String) meta = RT.map(
          RT.TAG_KEY, meta);
      else if (meta instanceof Keyword) meta = RT.map(meta, RT.T);
      else if (!(meta instanceof IPersistentMap)) throw new IllegalArgumentException(
          "Metadata must be Symbol,Keyword,String or Map");

      Object o = read(r, true, null, true);
      if (o instanceof IMeta) {
        if (line != -1 && o instanceof ISeq) {
          meta = ((IPersistentMap) meta).assoc(RT.LINE_KEY, line).assoc(
    ...
    static Object syntaxQuote(Object form) {
      ...
      if (Compiler.isSpecial(form)) ret = RT.list(Compiler.QUOTE, form);
        ...
        if (sym.ns == null && sym.name.endsWith("#")) {
          ...
          if (gmap == null) throw new IllegalStateException(
              "Gensym literal not in syntax-quote");
          Symbol gs = (Symbol) gmap.valAt(sym);
          if (gs == null) GENSYM_ENV.set(gmap.assoc(
              sym,
              gs = Symbol.intern(
                  null,
                  sym.name.substring(0, sym.name.length() - 1) + "__"
                      + RT.nextID() + "__auto__")));
          sym = gs;
        } else if (sym.ns == null && sym.name.endsWith(".")) {
          Symbol csym = Symbol.intern(null,
          ...
          if (sym.ns != null) maybeClass = Compiler.currentNS().getMapping(
              Symbol.intern(null, sym.ns));
          if (maybeClass instanceof Class) {
            // Classname/foo -> package.qualified.Classname/foo
            sym = Symbol.intern(((Class) maybeClass).getName(), sym.name);
          } else sym = Compiler.resolveSymbol(sym);
        }
        ret = RT.list(Compiler.QUOTE, sym);
      } else if (isUnquote(form)) return RT.second(form);
      else if (isUnquoteSplicing(form)) throw new IllegalStateException(
          "splice not in list");
      else if (form instanceof IPersistentCollection) {
        if (form instanceof IRecord) ret = form;
        else if (form instanceof IPersistentMap) {
          IPersistentVector keyvals = flattenMap(form);
          ret = RT.list(APPLY, HASHMAP,
              RT.list(SEQ, RT.cons(CONCAT, sqExpandList(keyvals.seq()))));
        } else if (form instanceof IPersistentVector) {
          ret = RT.list(APPLY, VECTOR, RT.list(SEQ,
              RT.cons(CONCAT, sqExpandList(((IPersistentVector) form).seq()))));
          ...
          ret = RT
              .list(
                  ...
                  RT.list(
                      SEQ,
                      RT.cons(CONCAT,
                          sqExpandList(((IPersistentSet) form).seq()))));
          ...
          || form instanceof Character || form instanceof String) ret = form;
      else ret = RT.list(Compiler.QUOTE, form);
      ...
      if (form instanceof IObj && RT.meta(form) != null) {
        ...
        if (newMeta.count() > 0) return RT.list(WITH_META, ret,
  ...
  public static class CharacterReader extends AFn {
    public Object invoke(Object reader, Object backslash) {
      ...
      if (token.length() == 1) return Character.valueOf(token.charAt(0));
        ...
        if (len > 3) throw Util
            .runtimeException("Invalid octal escape sequence length: " + len);
        int uc = readUnicodeChar(token, 1, len, 8);
        if (uc > 0377) throw Util
            .runtimeException("Octal escape sequence must be in range [0, 377].");
        return (char) uc;
      }
  ...
  /*
   * static class CtorReader extends AFn{ static final Symbol cls =
   * Symbol.intern("class");
   *
   * public Object invoke(Object reader, Object leftangle) { PushbackReader r =
   * (PushbackReader) reader; // #<class classname> // #<classname args*> //
   * #<classname/staticMethod args*> List list = readDelimitedList('>', r,
   * true); if(list.isEmpty()) throw Util.runtimeException(
   * "Must supply 'class', classname or classname/staticMethod"); Symbol s =
   * (Symbol) list.get(0); Object[] args = list.subList(1,
   * list.size()).toArray(); if(s.equals(cls)) { return
   * RT.classForName(args[0].toString()); } else if(s.ns != null) //static
   * method { String classname = s.ns; String method = s.name; return
   * Reflector.invokeStaticMethod(classname, method, args); } else { return
    ...
    public Object invoke(Object reader, Object eq) {
      ...
      if (o instanceof Symbol) {
        ...
        if (fs.equals(THE_VAR)) {
          ...
          return RT.var(vs.ns, vs.name); // Compiler.resolve((Symbol)
                                         // RT.second(o),true);
        }
        if (fs.name.endsWith(".")) {
          ...
          return Reflector
              .invokeConstructor(
                  RT.classForName(fs.name.substring(0, fs.name.length() - 1)),
                  args);
        }
      ...
      } else throw new IllegalArgumentException("Unsupported #= form");
    }
  }
  ...
  public static List readDelimitedList(char delim, PushbackReader r,
      boolean isRecursive) {
    final int firstline = (r instanceof LineNumberingPushbackReader) ? ((LineNumberingPushbackReader) r)
        .getLineNumber() : -1;
    ...
    for (;;) {
      ...
      if (ch == -1) {
        ...
        else throw Util.runtimeException("EOF while reading, starting at line "
            + firstline);
      }
  ...
  public static class CtorReader extends AFn {
    public Object invoke(Object reader, Object firstChar) {
      ...
      if (!(name instanceof Symbol)) throw new RuntimeException(
          "Reader tag must be a symbol");
      Symbol sym = (Symbol) name;
      return sym.getName().contains(".") ? readRecord(r, sym) : readTagged(r,
          sym);
    }
    ...
    private Object readTagged(PushbackReader reader, Symbol tag) {
      ...
      if (data_reader == null) {
        ...
        if (data_reader == null) {
          ...
          else throw new RuntimeException("No reader function for tag "
              + tag.toString());
        }
      }
    ...
    private Object readRecord(PushbackReader r, Symbol recordName) {
      ...
      if (ch == '{') {
        ...
        shortForm = false;
          ...
          .runtimeException("Unreadable constructor form starting with \"#"
              + recordName + (char) ch + "\"");
      ...
      if (shortForm) {
        ...
        for (Constructor ctor : allctors)
          if (ctor.getParameterTypes().length == recordEntries.length) ctorFound = true;

        if (!ctorFound) throw Util
            .runtimeException("Unexpected number of constructor arguments to "
        ...
        for (ISeq s = RT.keys(vals); s != null; s = s.next()) {
          if (!(s.first() instanceof Keyword)) throw Util
              .runtimeException("Unreadable defrecord form: key must be of type clojure.lang.Keyword, got "
                  + s.first().toString());
        }
        ret = Reflector.invokeStaticMethod(recordClass, "create",
            new Object[] {vals});
      }
  ...
  /*
   * public static void main(String[] args) throws Exception{ //RT.init();
   * PushbackReader rdr = new PushbackReader( new java.io.StringReader(
   * "(+ 21 21)" ) ); Object input = LispReader.read(rdr, false, new Object(),
   * false ); System.out.println(Compiler.eval(input)); }
   *
   * public static void main(String[] args){ LineNumberingPushbackReader r = new
   * LineNumberingPushbackReader(new InputStreamReader(System.in));
   * OutputStreamWriter w = new OutputStreamWriter(System.out); Object ret =
   * null; try { for(; ;) { ret = LispReader.read(r, true, null, false);
   * RT.print(ret, w); w.write('\n'); if(ret != null)
   * w.write(ret.getClass().toString()); w.write('\n'); w.flush(); } }
