...
public class Reflector {
  ...
  private static Throwable getCauseOrElse(Exception e) {
    if (e.getCause() != null) return e.getCause();
    return e;
  }

  private static RuntimeException throwCauseOrElseException(Exception e) {
    if (e.getCause() != null) throw Util.sneakyThrow(e.getCause());
  ...
  static Object invokeMatchingMethod(String methodName, List methods,
    ...
    } else // overloaded w/same arity
    {
      ...
      for (Iterator i = methods.iterator(); i.hasNext();) {
        ...
        if (isCongruent(params, args)) {
          if (foundm == null
              || Compiler.subsumes(params, foundm.getParameterTypes())) {
            foundm = m;
    ...
    if (m == null) throw new IllegalArgumentException(noMethodReport(
        methodName, target));
    ...
    if (!Modifier.isPublic(m.getDeclaringClass().getModifiers())) {
      ...
      if (m == null) throw new IllegalArgumentException(
          "Can't call public method of non-public class: " + oldm.toString());
    }
    try {
  ...
  public static Object invokeConstructor(Class c, Object[] args) {
    try {
      ...
      for (int i = 0; i < allctors.length; i++) {
        ...
        if (ctor.getParameterTypes().length == args.length) ctors.add(ctor);
      }
      if (ctors.isEmpty()) {
        throw new IllegalArgumentException("No matching ctor found" + " for "
            + c);
  ...
  public static Object setStaticField(String className, String fieldName,
      Object val) {
    Class c = RT.classForName(className);
  ...
  public static Object setInstanceField(Object target, String fieldName,
      Object val) {
    Class c = target.getClass();
    Field f = getField(c, fieldName, false);
    if (f != null) {
  ...
  public static Object invokeNoArgInstanceMember(Object target, String name) {
    ...
    if (meths.size() > 0) return invokeMatchingMethod(name, meths, target,
        RT.EMPTY_ARRAY);
  ...
  public static Object invokeInstanceMember(String name, Object target,
      Object arg1) {
    // check for field first
    Class c = target.getClass();
    Field f = getField(c, name, false);
  ...
  static public Field getField(Class c, String name, boolean getStatics) {
    ...
    for (int i = 0; i < allfields.length; i++) {
      if (name.equals(allfields[i].getName())
          && Modifier.isStatic(allfields[i].getModifiers()) == getStatics) return allfields[i];
    }
    return null;
  }
  ...
  static public List getMethods(Class c, int arity, String name,
        ...
        try {
          if (method.isBridge()
              && c.getMethod(method.getName(), method.getParameterTypes())
                  .equals(method)) bridgeMethods.add(method);
    ...
    if (methods.isEmpty()) methods.addAll(bridgeMethods);
  ...
  static Object boxArg(Class paramType, Object arg) {
    if (!paramType.isPrimitive()) return paramType.cast(arg);
    else if (paramType == boolean.class) return Boolean.class.cast(arg);
    else if (paramType == char.class) return Character.class.cast(arg);
    else if (arg instanceof Number) {
      Number n = (Number) arg;
      if (paramType == int.class) return n.intValue();
      else if (paramType == float.class) return n.floatValue();
      else if (paramType == double.class) return n.doubleValue();
      else if (paramType == long.class) return n.longValue();
      else if (paramType == short.class) return n.shortValue();
      else if (paramType == byte.class) return n.byteValue();
    }
  ...
  static public boolean paramArgTypeMatch(Class paramType, Class argType) {
    ...
    if (paramType == int.class) return argType == Integer.class
        || argType == long.class || argType == Long.class
        || argType == short.class || argType == byte.class;// || argType ==
                                                           // FixNum.class;
        ...
        || argType == int.class || argType == short.class
  ...
  public static Object prepRet(Class c, Object x) {
    if (!(c.isPrimitive() || c == Boolean.class)) return x;
    if (x instanceof Boolean) return ((Boolean) x) ? Boolean.TRUE
        : Boolean.FALSE;
