...
public class Namespace extends AReference implements Serializable {
  ...
  public Var intern(Symbol sym) {
    ...
    while ((o = map.valAt(sym)) == null) {
      if (v == null) v = new Var(this, sym);
      IPersistentMap newMap = map.assoc(sym, v);
      mappings.compareAndSet(map, newMap);
      map = getMappings();
    }
    if (o instanceof Var && ((Var) o).ns == this) return (Var) o;
  ...
  private void warnOrFailOnReplace(Symbol sym, Object o, Object v) {
    if (o instanceof Var) {
      ...
      if (ns == this) return;
      if (ns != RT.CLOJURE_NS) throw new IllegalStateException(sym
          + " already refers to: " + o + " in namespace: " + name);
    }
    RT.errPrintWriter().println(
        "WARNING: " + sym + " already refers to: " + o + " in namespace: "
            + name + ", being replaced by: " + v);
  }
  ...
  Class referenceClass(Symbol sym, Class val) {
    ...
    throw new IllegalStateException(sym + " already refers to: " + c
        + " in namespace: " + name);
  }
  ...
  public static Namespace remove(Symbol name) {
    if (name.equals(RT.CLOJURE_NS.name)) throw new IllegalArgumentException(
  ...
  public Var findInternedVar(Symbol symbol) {
    ...
    if (o != null && o instanceof Var && ((Var) o).ns == this) return (Var) o;
    return null;
  }
  ...
  public void addAlias(Symbol alias, Namespace ns) {
    if (alias == null || ns == null) throw new NullPointerException(
        "Expecting Symbol + Namespace");
    IPersistentMap map = getAliases();
    ...
    if (!map.valAt(alias).equals(ns)) throw new IllegalStateException("Alias "
        + alias + " already exists in namespace " + name + ", aliasing "
        + map.valAt(alias));
  }
